...
| [FlowActivityStream.test.ts](file:///home/munix/Desarrollo/GitRepo/Vibes/src/components/chat/FlowActivityStream.test.ts) | 3 | **Actividad flow.** Formatea duraciones sub-minuto/minuto y clampa valores negativos. |
| [FlowThinkBlock.test.ts](file:///home/munix/Desarrollo/GitRepo/Vibes/src/components/chat/FlowThinkBlock.test.ts) | 1 | **Pensamiento flow.** Verifica que el resumen usa el mismo formateador temporal que la actividad agrupada. |

**Cuándo usarlos:** Si se toca `FlowActivityStream`, `FlowThinkBlock`, el buffering de flow en `VibesMarkdownParser` o sus márgenes/espaciado, correr ambos tests focales.
