...
| [event_mapper.test.ts](file:///home/munix/Desarrollo/GitRepo/Vibes/src/ipc/runtime/event_mapper.test.ts) | 223 | Parity con OpenCode adapter: mapeo toolId → `<vibes-*>` tag, escape de atributos, extracción de detail/content, `<vibes-files-changed>`, `<vibes-token-usage>`, `<vibes-cancelled>`, `cleanResponseText` (REDACTED/thinking/think/assistant_response). **BUGFIX #122:** `session.failed` → `getFailedError()` expone el error serializado (o fallback genérico). |
...
| [permission_resolver.test.ts](file:///home/munix/Desarrollo/GitRepo/Vibes/src/ipc/runtime/permission_resolver.test.ts) | 16 | **Slice 3.3 — Cascada de prioridad.** 16 tests: defaults read_* → allow, mutación → ask, shell → ask. Pill global por tool. Sub-pill (rm deny wins over default shell ask). Custom rule (prefix `ls` allow, `rm /etc/` deny). Tool desconocida → ask (fail-closed). Args null → defaults. |
