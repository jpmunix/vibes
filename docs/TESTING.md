...
| [prompt_guard.test.ts](file:///home/munix/Desarrollo/GitRepo/Vibes/src/components/settings/prompt_guard.test.ts) | 9 | **Card #117 follow-up — prompt base no desactivable.** Regla pura `canDisablePrompt`: el `runtime_agent_base` no se puede desactivar (switch bloqueado con candado en Settings → Prompts); los `ctx_*` y los prompts custom sí. Protege el contrato de que el agente siempre tenga instrucciones base. **Card #183 — campos del editor.** `getPromptEditorLock(systemId)`: para el `runtime_agent_base` (prompt del sistema) el editor solo deja editable el CONTENIDO → título y descripción en solo lectura, y se ocultan categoría, ámbito (scope) y "Generar con IA"; los `ctx_*` y los prompts custom (sin systemId) conservan todos los campos editables. **Card #182 — aviso de verbosidad.** `isAgentCorePrompt(systemId)`: solo `runtime_agent_base` es el núcleo (los `ctx_*` y custom no); soporta el banner pasivo del editor que remite al selector de Verbosidad. |
