...
## Tests Unitarios / Integration (Vitest) — 34 archivos
...
| [ChatMessage.sticky.test.ts](file:///home/munix/Desarrollo/GitRepo/Vibes/src/components/chat/ChatMessage.sticky.test.ts) | 5 | **Card #227 — burbuja sticky: guard visual + scroll al inicio del mensaje.** (a) Guard estático: el wrapper sticky conserva `bg-background` opaco y `ChatMessage` no reintroduce un degradado absoluto con offset negativo bajo la burbuja (la franja errática). (b) `computeStickyScrollTop` (exportada): en el contenedor `flex flex-col-reverse`, `scrollTop=0` es el mensaje más nuevo y subir hace scrollTop negativo hasta `-(scrollHeight - clientHeight)`; se lee la posición natural vía `offsetTop` (inmune al sticky) y se convierte al valor invertido, clampeando a `[-maxScroll, 0]` para no irse al principio del chat. Test estático intencionado para la parte visual (combinación de clases que `happy-dom` no valida fiable); el math se verifica con invariantes de contención y monotonía porque `happy-dom` no aplica el layout column-reverse real. |

**Cuándo usarlos:** Si se toca `src/components/chat/normalizeMessageContent.ts`, el `useMemo normalizedMessageContent` o los tres puntos de render (`VibesMarkdownParser`/`UserMessageContent`) en `ChatMessage.tsx`, o si cambia el contrato de `stripDsmlToolCallBlocks` en el provider del runtime. Si se toca el sticky de mensajes de usuario, sus fondos, el overlay/degradado asociado o el scroll-al-inicio del mensaje (`handleScrollToNaturalTop` / `computeStickyScrollTop`) en `ChatMessage.tsx` o `MessagesList.tsx`, correr también `ChatMessage.sticky.test.ts`.
