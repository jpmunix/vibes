...
## Tests Unitarios / Integration (Vitest) — 36 archivos
...
| [router.test.ts](file:///home/munix/Desarrollo/GitRepo/Vibes/src/router.test.ts) | 9 | **Card #244 — fix 404 de Ctrl+R en app compilada (router con browser history).** `shouldUseHashHistory(protocol?)`: true bajo `file:` (Electron `loadFile`, sin SPA fallback), false para http/https (dev server Vite + web), false para protocolos desconocidos/blank, fallback a `window.location.protocol` con `vi.stubGlobal` (sin tocar `window.location` para no contaminar otros tests), y path defensivo SSR (sin `window` definido → false). `buildRouterHistory({ protocol? })`: instancia hash history bajo `file:` (URL reescrita a `#/chat?id=42`, sobrevive a recargas), `undefined` bajo http(s) (TanStack Router usa browser history por defecto), `createHref` idempotente, round-trip `createHref(/chat?id=42) → parseLocation` recupera `/chat?id=42` (simula el Ctrl+R), y guard de que `createHashHistory` se re-exporta desde `@tanstack/react-router` (anti-regresión si TanStack reorganiza internals). Guarda el switch de history para que el Ctrl+R en producción no pida `file:///chat` (404). |
...
| [runtime_host.contextdebug.test.ts](file:///home/munix/Desarrollo/GitRepo/Vibes/src/ipc/runtime/runtime_host.contextdebug.test.ts) | 4 | **Ventana de depuración de contexto (temporal).** `setDebugContext` togglea el flag del `LoopConfig` (el loop lo lee por referencia → hot). `setContextDebugWindow`: `null` → flag OFF y sin sender; ventana viva → flag ON y sender accesible; ventana destruida → sender `null` y flag OFF. Electron/settings/model_resolver mockeados; no abre SQLite (prueba la mutación del estado del debug, no el runtime). Cubre la lógica de "abrir ventana = debug ON, cerrar = debug OFF" que evita des-sincronizar un toggle manual. |
| [context_debug_log.test.ts](file:///home/munix/Desarrollo/GitRepo/Vibes/src/ipc/runtime/context_debug_log.test.ts) | 7 | **Persistencia del log de contexto (JSONL en userData).** Append escribe una línea que read recupera; append acumula en orden; read con archivo ausente → `[]` sin lanzar; read salta líneas corruptas (crash a mitad de write); clear borra el log y es idempotente (archivo ausente no lanza); getPath apunta a `userData/context-debug.log`. Electron mockeado (getPath → tmpdir real por test); fs de verdad corre — es el contrato que garantiza la persistencia que sobrevive reinicios. |
...
| [FlowActivityStream.test.ts](file:///home/munix/Desarrollo/GitRepo/Vibes/src/components/chat/FlowActivityStream.test.ts) | 6 | **Actividad flow unificada (pensamiento + tools).** `formatActivityDuration` (duraciones sub-minuto/minuto, clamp de negativos) y `parseDurationMs` (parsea el atributo `duration-ms` de los tags; sin atributo → `undefined` para mensajes históricos; valores basura → `undefined`, nunca inventa duración). El módulo suma las duraciones reales — cero `Date.now()` en la UI, el toggle no puede mover el número. |

**Cuándo usarlos:** Si se toca `FlowActivityStream`, el buffering unificado de flow en `VibesMarkdownParser` o sus márgenes/espaciado, correr el test focal. `FlowThinkBlock` fue absorbido por `FlowActivityStream` (2026-09-04): sus tests viven ahora en el mismo archivo.

### Vista del chat — fallback Max (`full`) → `flow` (2026-09-07)

| Archivo | Líneas | Qué cubre |
|---|---|---|
| [chatAtoms.test.ts](file:///home/munix/Desarrollo/GitRepo/Vibes/src/atoms/chatAtoms.test.ts) | 8 | **Fallback del modo "Max" oculto de la UI.** Tras quitar "Max" del selector de Ajustes y hacer de `flow` el default, el valor `full` sigue existiendo en el schema pero se normaliza a `flow` en un solo punto: `effectiveChatRenderModeAtom`. 8 tests (jotai `createStore`, sin montar React): `full`→`flow`, `flow`→`flow`, `zen`→`zen` (se conserva), `undefined`→`flow`, `null` (settings sin cargar)→`flow`; y los derivados `isZenModeAtom`/`isFlowModeAtom` (`full` y `flow` → render mínimo + flow; `zen` → render mínimo sin flow). Guarda el contrato "quien tenía Max se ve como Flow, quien tenía Zen se queda en Zen". |

**Cuándo usarlos:** Si se toca `src/atoms/chatAtoms.ts` (los atoms de modo de render) o la lógica de normalización del modo de vista. Cambiar la política de fallback (p. ej. reintroducir `full` a la UI o cambiar el default) requiere actualizar estos tests.
