...
export default tool({
  ...
  args: {
    ...
    old_string: tool.schema
      .string()
      .describe("Exact string to find in the file"),
  ...
  async execute(args, context) {
    ...
    if (!rawPath) {
      throw new Error(
        "No file path provided. Got args: " + Object.keys(args).join(", "),
      );
    }

    // Handle absolute vs relative paths
    const fullPath = nodePath.isAbsolute(rawPath)
      ? rawPath
      : nodePath.join(context.directory, rawPath);
