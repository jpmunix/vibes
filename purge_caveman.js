...
async function run() {
  ...
  const res1 = await client.execute(
    "DELETE FROM prompts WHERE system_id = 'ctx_caveman_mode'",
  );
  ...
  const res2 = await client.execute(
    "DELETE FROM prompts WHERE content LIKE '%MODO HOMBRE DE LAS CAVERNAS%'",
  );
  console.log(
    "Deleted any remaining prompts containing caveman text:",
    res2.rowsAffected,
  );
}
