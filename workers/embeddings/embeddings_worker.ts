...
if (workerData?.cacheDir) {
  ...
  logger.debug(`[EMBEDDINGS WORKER] Cache directory set to: ${workerData.cacheDir}`);
} else {
...
parentPort.on("message", async (input: EmbeddingsWorkerInput) => {
  if (input.type === "generateEmbeddings") {
    try {
      logger.debug(`[EMBEDDINGS WORKER] Processing ${input.chunks.length} chunks for ${input.filePath}`);
      ...
      for (let i = 0; i < input.chunks.length; i++) {
        ...
        if ((i + 1) % 3 === 0 || i === input.chunks.length - 1) {
          logger.debug(`[EMBEDDINGS WORKER] Processed ${i + 1}/${input.chunks.length} chunks`);
        }
      }

      const duration = Date.now() - startTime;
      logger.debug(`[EMBEDDINGS WORKER] Completed ${input.filePath} in ${duration}ms`);
      ...
      logger.error(`[EMBEDDINGS WORKER] Error processing ${input.filePath}:`, error);
