  ...
  try {
    ...
    const response = await fetch(allReleasesUrl, {
      headers: {
        ...
        "User-Agent": "vibes-release-verifier",
      },
    });
    ...
    const expectedAssets = [
      `vibes-${normalizeVersionForPlatform(version, "rpm")}-1.x86_64.rpm`,
      `vibes-${normalizeVersionForPlatform(version, "nupkg")}-full.nupkg`,
      `vibes-${version}.Setup.exe`,
      `vibes-darwin-arm64-${version}.zip`,
      `vibes-darwin-x64-${version}.zip`,
      `vibes_${normalizeVersionForPlatform(version, "deb")}_amd64.deb`,
      `vibes_${version}_x86_64.AppImage`,
      "RELEASES",
    ];
