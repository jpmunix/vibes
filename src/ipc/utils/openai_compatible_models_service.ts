...
async function writeCacheToDisk(
  providerId: string,
  cache: CachedModelsFile,
): Promise<void> {
  try {
    await fs.writeFile(
      getCacheFilePath(providerId),
...
function extractPricing(model: RawModel): {
  ...
  if (model.pricing) {
    ...
    if (p.prompt !== undefined && p.completion !== undefined) {
      const promptPerToken =
        typeof p.prompt === "string" ? parseFloat(p.prompt) : p.prompt;
      const completionPerToken =
        typeof p.completion === "string"
          ? parseFloat(p.completion)
          : p.completion;
      if (!isNaN(promptPerToken)) inputCost = promptPerToken * 1_000_000;
      if (!isNaN(completionPerToken))
        outputCost = completionPerToken * 1_000_000;
    }
  ...
  return {
    pricingInput:
      inputCost !== null ? `${sym}${inputCost.toFixed(2)}/M` : undefined,
    pricingOutput:
      outputCost !== null ? `${sym}${outputCost.toFixed(2)}/M` : undefined,
    dollarSigns,
  };
}
...
function extractMaxOutputTokens(model: RawModel): number | undefined {
  return model.max_output_tokens || model.max_completion_tokens || undefined;
}
...
function deriveTag(model: RawModel): { tag?: string; tagColor?: string } {
  ...
  if (tagSet.has("vision") || tagSet.has("image"))
    return { tag: "Vision", tagColor: "green" };
  if (tagSet.has("tools") || tagSet.has("function_calling"))
    return { tag: "Tools", tagColor: "orange" };
...
function transformModel(model: RawModel): ModelOption {
  ...
  return {
    ...
    description,
    temperature: 0,
    ...(pricing.dollarSigns !== undefined
      ? { dollarSigns: pricing.dollarSigns }
      : {}),
...
export async function fetchCompatibleModels(
  ...
  if (isCacheValid(diskCache)) {
    ...
    logger.info(
      `Loaded ${diskCache!.models.length} cached models for ${providerId} from disk`,
    );
  ...
  if (diskCache && diskCache.models.length > 0) {
    memoryCache.set(providerId, diskCache);
    logger.info(
      `Returning stale cache (${diskCache.models.length} models) for ${providerId}, refreshing...`,
    );
...
async function refreshFromAPI(
  ...
  try {
    ...
    logger.info(
      `Fetching models from ${modelsUrl} for provider ${providerId}...`,
    );
    ...
    if (sample?.context_size || sample?.context_length)
      enrichedFields.push("context");
    if (sample?.tags) enrichedFields.push("tags");
    if (sample?.description) enrichedFields.push("description");
    if (enrichedFields.length > 0) {
      logger.info(
        `Provider ${providerId} returns enriched fields: [${enrichedFields.join(", ")}]`,
      );
    }
