 ...
 */
export async function migrateLegacyTypesToSession(
  userId: string,
): Promise<number> {
  try {
    ...
    logger.info(
      `[Migration] Migrated ${legacyCount} memories from legacy types (fact/episode/decision) → session for user ${userId.slice(0, 8)}...`,
    );
    return legacyCount;
  } catch (error: any) {
...
export async function transitionIssue(
  ...
  try {
    ...
    logger.info(
      `[Issue] Transitioned id=${memoryId}: ${currentStatus} → ${newStatus}`,
    );
    return true;
  } catch (error: any) {
    logger.warn(
      `[Issue] Transition failed for id=${memoryId}: ${error.message}`,
    );
    return false;
  }
}
...
export async function confirmMemory(
  ...
  try {
    ...
    logger.info(
      `[Confirm] Memory id=${memoryId} confirmed — source=manual, importance=80`,
    );
    return true;
  } catch (error: any) {
