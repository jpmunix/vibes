  ...
  try {
    ...
    if (userPrompt) {
      ...
      if (!injectionGuard.allowed) {
        debugLog("InjectionGuard", `❌ Rejected: ${injectionGuard.reason}`, {
          promptExcerpt: userPrompt.slice(0, 80),
        });
        logger.info(`[Memory] Skipped injection: ${injectionGuard.reason}`);
        return EMPTY_RESULT;
      }
      debugLog("InjectionGuard", `✅ Approved`, {
        promptLength: userPrompt.length.toString(),
      });
    }
    ...
    debugLog("Router", `Loaded memory pool`, {
      candidates: rows.length.toString(),
    ...
    const lines = selectedRows.map((row) => {
      ...
      const statusSuffix =
        row.type === "issue" && row.status ? `:${row.status}` : "";
    ...
    logger.info(
      `[Memory] Context built: ${selectedRows.length} memories (Router: ${userPrompt ? "yes" : "fallback"}) for appId=${appId}`,
    );
...
async function routerSelect(
  ...
  try {
    ...
    const memoryList = memories
      .map(
        (m) =>
          `- [#${m.id}] [${m.type}] key:${m.key || "—"} | imp:${m.importance} | ${m.content}`,
      )
      .join("\n");

    const rawSelectionPrompt = await getSystemPrompt(
      "memory_selection",
      settings.userId,
    );
    const selectionPrompt = rawSelectionPrompt.replace(
      "__NUM_MEMORIES__",
      String(maxSelection),
    );
    ...
    if (recentMessages && recentMessages.length > 0) {
      ...
      conversationContext = ["## Prompt del Usuario:", userPrompt].join("\n");
    }
    ...
    try {
      ...
      if (parsed && parsed.ids && Array.isArray(parsed.ids)) {
        ...
        logger.warn(
          "[Memory] Router returned unexpected structure:",
    ...
    debugLog(
      "Router",
      `✅ Router selected ${selected.length}/${memories.length}`,
      {
    ...
    logger.info(
      `[Memory] Router selected ${selected.length}/${memories.length} memories: [${selectedIds.join(", ")}]`,
    );
    return selected;
  } catch (error: any) {
    logger.warn(
      `[Memory] Router call failed: ${error.message} — falling back to top-N`,
    );
    logPipelineCall({
      userId,
      appId,
