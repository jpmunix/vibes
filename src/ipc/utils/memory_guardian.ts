...
export type GuardianInjectionRejectReason = "trivial_ack" | "short_no_tech";
...
const TECH_PATTERNS = {
  ...
  syntax:
    /\b(class|function|const|let|var|import|export|async|await|interface|type|struct|enum|public|private|protected|namespace|trait|impl|fn|yield|return|throw|catch|try|promise|callback|middleware|hook|component|props|state|useState|useEffect|useCallback|useMemo|useRef)\b/i,
  ...
  network:
    /(\b(get|post|put|patch|delete)\b\s+\/|https?:\/\/|\b(http|https|fetch|axios|graphql|rest|webhook|websocket|socket\.io|docker|kubernetes|aws|lambda|nginx|apache|vercel|netlify|cloudflare|dns|ssl|cors|jwt|oauth|token|endpoint|payload|json|api|grpc|cdn)\b)/i,

  // Technology stack
  stack:
    /\b(react|vue|angular|svelte|next\.js|nuxt|node|deno|bun|typescript|javascript|php|python|go|rust|java|c#|c\+\+|tailwind|bootstrap|sass|less|vite|webpack|esbuild|jest|cypress|playwright|vitest|git|bash|linux|ubuntu|npm|yarn|pnpm|electron|flutter|swift|kotlin)\b/i,
