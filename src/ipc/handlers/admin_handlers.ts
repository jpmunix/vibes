...
export function registerAdminHandlers(): void {
    ...
    async (_event, _input, context) => {
      ...
      const lastMsgRows = await db
        .select({
          ...
          lastMessageAt: max(remoteSchema.messages.createdAt).as(
            "last_message_at",
          ),
        })
    ...
    async (_event, _input, context) => {
      ...
      const users = userRows
        .map((user) => {
          ...
          const apps = userApps
            .map((app) => {
              const appMems = memoryRows.filter(
                (r) => r.userId === user.id && r.appId === app.id,
              );
  ...
  createTypedHandler(
    ...
    async (_event, input, context) => {
      ...
      const apps = userApps.filter(
        (a) => telemetryAppIds.has(a.id) || pipelineAppIds.has(a.id),
      );
      ...
      const telWhere = filterApp
        ? and(
            ...
            eq(remoteSchema.memoryTelemetry.appId, appId),
          )
        : eq(remoteSchema.memoryTelemetry.userId, userId);
      ...
      const stats = Object.entries(statsMap).map(([action, count]) => ({
        action,
        count,
      }));
      ...
      const plWhere = filterApp
        ? and(
            ...
            eq(remoteSchema.memoryPipelineLogs.appId, appId),
          )
        : eq(remoteSchema.memoryPipelineLogs.userId, userId);
  ...
  createTypedHandler(
    ...
    async (_event, input, context) => {
      ...
      const chat = await db.query.chats.findFirst({
        ...
        with: {
          messages: {
            orderBy: (messages: any, ops: any) => [
              ops.asc(messages.createdAt),
              ops.asc(messages.id),
            ],
          },
        },
      ...
      const { normalizeLegacyTags } =
        await import("../../../shared/normalizeLegacyTags");
  ...
  createTypedHandler(
    ...
    async (_event, input, context) => {
      ...
      const promptRows = await db
        ...
        .leftJoin(
          remoteSchema.promptsCategories,
          eq(
            remoteSchema.prompts.categoryId,
            remoteSchema.promptsCategories.id,
          ),
        )
  ...
  createTypedHandler(
    ...
    async (_event, input, context) => {
      ...
      if (input.key.startsWith("prompt:")) {
        ...
        logger.info(
          `Admin updated prompt: ${systemId} for user ${input.userId}`,
        );
        return { success: true };
      }
      ...
      logger.info(
        `Admin set preference: ${input.key} for user ${input.userId}`,
      );
      return { success: true };
    },
  );
  ...
  createTypedHandler(
    ...
    async (_event, input, context) => {
      ...
      logger.info(
        `Admin deleted preference: ${input.key} for user ${input.userId}`,
      );
      return { success: true };
    },
  );
  ...
  createTypedHandler(
    ...
    async (_event, input, context) => {
      ...
      if (sourceRows.length === 0) {
        throw new Error(
          "No se encontraron preferencias del usuario origen para las claves seleccionadas",
        );
      }
      ...
      for (const targetUserId of targetUserIds) {
        ...
        if (mode === "overwrite") {
          ...
          for (const row of sourceRows) {
            ...
            await db.insert(remoteSchema.userPreferences).values({
              userId: targetUserId,
              appId: 0,
              key: row.key,
              value: row.value,
      ...
      logger.info(
        `Admin copied ${written} preferences (skipped ${skipped}) from ${sourceUserId} to ${targetUserIds.length} user(s)`,
      );
