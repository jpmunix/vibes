import { db } from "../../db";
import { debates, debateMessages, debateTags, debateToTags } from "../../db/schema";
...
export function registerDebateHandlers() {
    ...
    createTypedHandler(debateContracts.createDebate, async (_, { title, tagIds }) => {
    ...
    createTypedHandler(debateContracts.updateDebate, async (_, { id, title, summary }) => {
        await db
            .update(debates)
            .set({
                ...(title && { title }),
    ...
    createTypedHandler(debateContracts.updateMessage, async (_, { id, content, injectedItems }) => {
        await db
    ...
    createTypedHandler(debateContracts.createTag, async (_, { name, color }) => {
        const [tag] = await db.insert(debateTags).values({ name, color }).returning();
        return tag;
    });

    createTypedHandler(debateContracts.addTagToDebate, async (_, { debateId, tagId }) => {
        await db.insert(debateToTags).values({ debateId, tagId }).onConflictDoNothing();
    });

    createTypedHandler(debateContracts.removeTagFromDebate, async (_, { debateId, tagId }) => {
        await db
            .delete(debateToTags)
            .where(and(eq(debateToTags.debateId, debateId), eq(debateToTags.tagId, tagId)));
    });
    ...
    createTypedHandler(debateContracts.summarizeDebate, async (_, debateId) => {
        ...
        const content = debate.messages.map((m) => `${m.role}: ${m.content}`).join("\n\n");
