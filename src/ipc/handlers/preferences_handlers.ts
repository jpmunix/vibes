...
export function registerPreferencesHandlers() {
  ...
  createTypedHandler(
    miscContracts.getPreference,
    async (_, { key, appId }, context) => {
  ...
  createTypedHandler(
    miscContracts.setPreference,
    async (event, { key, value, appId }, context) => {
      if (!context.userId) throw new Error("Unauthorized");
  ...
  createTypedHandler(
    miscContracts.getPreferences,
    async (_, { keys, appId }, context) => {
      if (!context.userId) throw new Error("Unauthorized");
  ...
  createTypedHandler(
    miscContracts.hydratePreferences,
    async (_, __, context) => {
      if (!context.userId) throw new Error("Unauthorized");
