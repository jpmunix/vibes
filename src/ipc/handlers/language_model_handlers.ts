...
export function registerLanguageModelHandlers() {
  createTypedHandler(
    languageModelContracts.getProviders,
  ...
  createTypedHandler(
    ...
    async (_event, params, context) => {
      ...
      if (existing.length > 0) {
        await db
          .update(remoteSchema.languageModels)
          .set({
        ...
        logger.info(
          `[CustomModel] Updated: ${params.apiName} → ${params.displayName}`,
        );
      } else {
        ...
        logger.info(
          `[CustomModel] Created: ${params.apiName} → ${params.displayName}`,
        );
      }
    },
  );
  ...
  createTypedHandler(
    languageModelContracts.getModels,
    async (_event, params, context) => {
  ...
  createTypedHandler(
    languageModelContracts.getModelsByProviders,
    async (_event, _input, context) => {
  ...
  createTypedHandler(
    ...
    async (_event, params) => {
      ...
      logger.info(
        `Manual refresh of custom provider models: ${params.providerId}`,
      );
      const settings = readSettings();
      const config = settings.customProviders?.find(
        (p) => p.id === params.providerId,
      );
      if (!config)
        throw new Error(`Custom provider "${params.providerId}" not found`);

      const { clearCompatibleModelsCache, fetchCompatibleModels } =
        await import("../utils/openai_compatible_models_service");
      clearCompatibleModelsCache(params.providerId);
      await fetchCompatibleModels(
        params.providerId,
        config.apiBaseUrl,
        config.apiKey?.value,
      );
