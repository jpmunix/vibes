    ...
    if (isDirectory) {
      ...
      if (fs.existsSync(filePath)) {
        ...
        const relativePath = path
          .relative(rootBasePath, filePath)
          .replace(/\\/g, "/");
...
export function registerDocsHandlers() {
  ...
  createTypedHandler(
    systemContracts.getDocPage,
    async (_, { relativePath, baseDir }) => {
      const basePath = getDocsBasePath(baseDir);
  ...
  createTypedHandler(
    systemContracts.searchDocs,
    async (_, { query, baseDir }) => {
      ...
      for (const filePath of mdFiles) {
        ...
        if (matchIdx !== -1) {
          ...
          const originalMatchText = cleanBody.slice(
            matchIdx,
            matchIdx + queryNorm.length,
          );
          ...
          const end = Math.min(
            cleanBody.length,
            matchIdx + actualMatchLength + SNIPPET_RADIUS,
          );
