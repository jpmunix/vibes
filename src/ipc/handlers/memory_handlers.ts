...
import {
  decayMemories,
  migrateLegacyTypesToSession,
  compactOldSessions,
} from "../utils/memory_lifecycle";
...
export function registerMemoryHandlers(): void {
  ...
  createTypedHandler(
    memoryContracts.getMemories,
    async (_event, appId, ctx) => {
  ...
  // ── CREATE MEMORY ────────────────────────────────────────────────────
  createTypedHandler(
    memoryContracts.createMemory,
    async (_event, params, ctx) => {
      const db = getRemoteDb();
      const userId = ctx.userId;
      if (!userId) throw new Error("Unauthorized");
      ...
      logger.info(
        `[Memory] Created: type=${params.type} key=${params.key ?? "—"} appId=${params.appId} id=${inserted.id}`,
      );
      return inserted.id;
    },
  );
  ...
  // ── DELETE MEMORY ────────────────────────────────────────────────────
  createTypedHandler(
    memoryContracts.deleteMemory,
    async (_event, memoryId, ctx) => {
      const db = getRemoteDb();
      const userId = ctx.userId;
      if (!userId) throw new Error("Unauthorized");
  ...
  // ── GET MEMORY CONTEXT ───────────────────────────────────────────────
  createTypedHandler(
    memoryContracts.getMemoryContext,
    async (_event, appId, ctx) => {
      const userId = ctx.userId;
      if (!userId) throw new Error("Unauthorized");
  ...
  createTypedHandler(
    memoryContracts.extractMemories,
    async (_event, _params, _ctx) => {
      return [];
    },
  );
  ...
  createTypedHandler(
    memoryContracts.condenseSessionMemories,
    async (_event, { appId, chatId }, ctx) => {
      const userId = ctx.userId;
      if (!userId) throw new Error("Unauthorized");
  ...
  createTypedHandler(
    memoryContracts.getAllMemories,
    async (_event, _input, ctx) => {
      const db = getRemoteDb();
      const userId = ctx.userId;
      if (!userId) throw new Error("Unauthorized");
  ...
  createTypedHandler(
    memoryContracts.deleteAllMemories,
    async (_event, appId, ctx) => {
      const db = getRemoteDb();
      const userId = ctx.userId;
      if (!userId) throw new Error("Unauthorized");
      ...
      logger.info(
        `[Memory] Deleted all: ${existing.length} memories for appId=${appId}`,
      );
  ...
  createTypedHandler(
    memoryContracts.getMemoryTelemetryStats,
    async (_event, appId, ctx) => {
      const db = getRemoteDb();
      const userId = ctx.userId;
      if (!userId) throw new Error("Unauthorized");
  ...
  createTypedHandler(
    memoryContracts.getMemoryTelemetryRecent,
    async (_event, appId, ctx) => {
      const db = getRemoteDb();
      const userId = ctx.userId;
      if (!userId) throw new Error("Unauthorized");

      const conditions = [eq(remoteSchema.memoryTelemetry.userId, userId)];
  ...
  // ── PIPELINE LOGS (RAW) ─────────────────────────────────────────────
  createTypedHandler(
    memoryContracts.getPipelineLogs,
    async (_event, params, ctx) => {
      const db = getRemoteDb();
      const userId = ctx.userId;
      if (!userId) throw new Error("Unauthorized");

      const conditions = [eq(remoteSchema.memoryPipelineLogs.userId, userId)];

      if (params.appId && params.appId > 0) {
        conditions.push(
          eq(remoteSchema.memoryPipelineLogs.appId, params.appId),
        );
      }

      if (params.stage) {
        conditions.push(
          eq(remoteSchema.memoryPipelineLogs.stage, params.stage),
        );
      }
  ...
  // ── PURGE ALL STATS ──────────────────────────────────────────────────
  createTypedHandler(
    memoryContracts.purgeAllMemoryStats,
    async (_event, _input, ctx) => {
      const db = getRemoteDb();
      const userId = ctx.userId;
      if (!userId) throw new Error("Unauthorized");
      ...
      logger.info(
        `[Memory] Purged all stats: ${telemetryDeleted} telemetry + ${pipelineLogsDeleted} pipeline logs`,
      );
  ...
  createTypedHandler(
    memoryContracts.getAppsWithAnalyzerData,
    async (_event, _input, ctx) => {
      const db = getRemoteDb();
      const userId = ctx.userId;
      if (!userId) throw new Error("Unauthorized");
  ...
  createTypedHandler(
    memoryContracts.bootstrapProjectMemories,
    async (_event, _params, ctx) => {
      const userId = ctx.userId;
      if (!userId) throw new Error("Unauthorized");
  ...
  createTypedHandler(
    memoryContracts.compactMemories,
    async (_event, params, ctx) => {
      const userId = ctx.userId;
      if (!userId) throw new Error("Unauthorized");
  ...
  // ── GET DEBUG LOGS ──────────────────────────────────────────────────
  createTypedHandler(
    memoryContracts.getDebugLogs,
    async (_event, params, ctx) => {
      const db = getRemoteDb();
      const userId = ctx.userId;
      if (!userId) throw new Error("Unauthorized");

      const conditions = [eq(remoteSchema.memoryDebugLogs.userId, userId)];
  ...
  createTypedHandler(
    memoryContracts.purgeDebugLogs,
    async (_event, _input, ctx) => {
      const db = getRemoteDb();
      const userId = ctx.userId;
      if (!userId) throw new Error("Unauthorized");
  ...
  restorePendingBuffers().catch((e) =>
    logger.warn(`[Memory] Buffer restoration failed: ${e.message}`),
  );
  ...
  (async () => {
    try {
      ...
      await db
        .delete(remoteSchema.streamTasks)
        .where(
          and(
      ...
      logger.info(
        "[Memory] Startup cleanup: purged stale pipeline logs, telemetry, debug logs, inactive memories, and old stream tasks (>7d)",
      );
    } catch (e: any) {
  ...
  (async () => {
    try {
      ...
      logger.info(
        `[Memory] One-shot legacy purge: deleted ${deleted} non-manual-preference memories`,
      );
    } catch (e: any) {
