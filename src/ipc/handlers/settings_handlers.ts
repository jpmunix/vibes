...
export function registerSettingsHandlers() {
  ...
  createTypedHandler(
    settingsContracts.getUserSettings,
    async (_, __, context) => {
  ...
  createTypedHandler(
    settingsContracts.setUserSettings,
    async (event, settings, context): Promise<UserSettings> => {
      if (context.userId && preferencesCache.isHydrated) {
      ...
      for (const win of BrowserWindow.getAllWindows()) {
        if (
          !win.isDestroyed() &&
          win.webContents &&
          win.webContents.id !== senderWebContentsId
        ) {
  ...
  const getGlobalSkillsDir = () =>
    path.join(app.getPath("userData"), "opencode-config", "skills");
  ...
  createTypedHandler(
    settingsContracts.renameGlobalSkill,
    async (event, { oldPath, newPath }) => {
      ...
      if (!isSafePath(fullOld) || !isSafePath(fullNew)) {
        throw new Error(
          "Acceso no autorizado fuera del directorio de skills globales.",
        );
      }
  ...
  createTypedHandler(
    settingsContracts.readGlobalSkill,
  ...
  createTypedHandler(
    settingsContracts.editGlobalSkill,
  ...
  createTypedHandler(
    settingsContracts.deleteGlobalSkill,
    async (event, { filePath }) => {
      const baseDir = getGlobalSkillsDir();
      const fullPath = path.join(baseDir, filePath);
