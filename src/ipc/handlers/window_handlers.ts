...
const pendingChatPrompts = new Map<
  number,
  {
    prompt: string;
    attachments?: Array<{
...
export function registerWindowHandlers() {
  ...
  createTypedHandler(
    systemContracts.openDatabaseWindow,
    async (event, { appId }) => {
      ...
      const saved = getSavedWindowBounds("database", {
        width: 1000,
        height: 700,
      });

      const dbIconPath = path.join(
        app.getAppPath(),
      ...
      dbWindow.on("close", () =>
        saveSecondaryWindowState("database", dbWindow),
      );
      dbWindow.on("closed", () => {
  ...
  createTypedHandler(
    systemContracts.openGitWindow,
    async (event, { appId, commitHash, theme, themeIntensity }) => {
      // If a window for this appId already exists, focus it
      ...
      const gitIconPath = path.join(
        app.getAppPath(),
        `assets/${getActiveFlavor().iconFolder}/logo.png`,
      );
      const gitIcon = nativeImage.createFromPath(gitIconPath);

      const savedGit = getSavedWindowBounds("git", {
        width: 1100,
        height: 750,
      });
      ...
      logger.info(
        `Opened git viewer window for app ${appId}${commitHash ? `, commit ${commitHash}` : ""}`,
      );
    },
  );
  ...
  createTypedHandler(
    systemContracts.openChatWindow,
    async (
      event,
      { appId, chatId, prompt, chatMode, attachments, theme, themeIntensity },
    ) => {
      ...
      const savedChat = getSavedWindowBounds("chat", {
        width: 1200,
        height: 800,
      });

      const chatIconPath = path.join(
        app.getAppPath(),
        `assets/${getActiveFlavor().iconFolder}/logo.png`,
      );
      ...
      chatWindow.on("close", () =>
        saveSecondaryWindowState("chat", chatWindow),
      );
      ...
      logger.info(
        `Opened chat window for app ${appId}${chatId ? `, chat ${chatId}` : ""}`,
      );
    },
  );
  ...
  createTypedHandler(
    systemContracts.openMessageWindow,
    async (
      event,
      { appId, chatId, messageId, theme, themeIntensity },
      context,
    ) => {
      ...
      const savedMsg = getSavedWindowBounds("message", {
        width: 800,
        height: 600,
      });

      const msgIconPath = path.join(
        app.getAppPath(),
        `assets/${getActiveFlavor().iconFolder}/logo.png`,
      );
      ...
      messageWindow.on("close", () =>
        saveSecondaryWindowState("message", messageWindow),
      );
  ...
  createTypedHandler(
    systemContracts.getPendingChatPrompt,
    async (_event, chatId) => {
  ...
  createTypedHandler(
    systemContracts.openConsoleWindow,
    async (event, { appId, theme, themeIntensity }) => {
      // If a window for this appId already exists, focus it
      ...
      const savedConsole = getSavedWindowBounds("console", {
        width: 900,
        height: 550,
      });

      const consoleIconPath = path.join(
        app.getAppPath(),
        `assets/${getActiveFlavor().iconFolder}/logo.png`,
      );
      ...
      consoleWindow.on("close", () =>
        saveSecondaryWindowState("console", consoleWindow),
      );
  ...
  createTypedHandler(
    systemContracts.openCodeWindow,
    async (event, { appId, theme, themeIntensity }) => {
      // If a window for this appId already exists, focus it
      ...
      const codeIconPath = path.join(
        app.getAppPath(),
        `assets/${getActiveFlavor().iconFolder}/logo.png`,
      );
      const codeIcon = nativeImage.createFromPath(codeIconPath);

      const savedCode = getSavedWindowBounds("code", {
        width: 1100,
        height: 750,
      });
      ...
      codeWindow.on("close", () =>
        saveSecondaryWindowState("code", codeWindow),
      );
  ...
  createTypedHandler(
    systemContracts.navigateMainWindow,
    async (_event, { route, search }) => {
      // Find the main window — it's the one NOT tracked as chat/db/git/console/code
      const trackedWindows = new Set<number>();
      for (const w of chatWindows.values())
        if (!w.isDestroyed()) trackedWindows.add(w.id);
      for (const w of databaseWindows.values())
        if (!w.isDestroyed()) trackedWindows.add(w.id);
      for (const w of gitWindows.values())
        if (!w.isDestroyed()) trackedWindows.add(w.id);
      for (const w of consoleWindows.values())
        if (!w.isDestroyed()) trackedWindows.add(w.id);
      for (const w of codeWindows.values())
        if (!w.isDestroyed()) trackedWindows.add(w.id);
      for (const w of messageWindows.values())
        if (!w.isDestroyed()) trackedWindows.add(w.id);
      for (const w of memoryWindows.values())
        if (!w.isDestroyed()) trackedWindows.add(w.id);
      if (adminWindow && !adminWindow.isDestroyed())
        trackedWindows.add(adminWindow.id);
      if (playgroundWindow && !playgroundWindow.isDestroyed())
        trackedWindows.add(playgroundWindow.id);
      if (docsWindow && !docsWindow.isDestroyed())
        trackedWindows.add(docsWindow.id);
  ...
  createTypedHandler(
    systemContracts.sendConsoleLogToChat,
    async (_event, { appId, formattedLog }) => {
      ...
      if (mainWindow) {
        ...
        logger.warn(
          "sendConsoleLogToChat: no target window found for appId " + appId,
        );
      }
    },
  );
  ...
  createTypedHandler(
    systemContracts.openMemoryWindow,
    async (event, { appId, theme, themeIntensity }) => {
      ...
      const savedMemory = getSavedWindowBounds("memory", {
        width: 900,
        height: 650,
      });

      const memIconPath = path.join(
        app.getAppPath(),
        `assets/${getActiveFlavor().iconFolder}/logo.png`,
      );
      ...
      memoryWindow.webContents.on("context-menu", (_e, params) => {
        ...
        menu.append(
          new MenuItem({
            label: "Inspect Element",
            click: () =>
              memoryWindow.webContents.inspectElement(params.x, params.y),
          }),
        );
        menu.popup();
      });
      ...
      memoryWindow.on("close", () =>
        saveSecondaryWindowState("memory", memoryWindow),
      );
  ...
  createTypedHandler(
    systemContracts.openAdminWindow,
    async (event, { theme, themeIntensity }) => {
      ...
      const savedAdmin = getSavedWindowBounds("admin", {
        width: 1000,
        height: 700,
      });
      ...
      adminWindow.webContents.on("context-menu", (_e, params) => {
        ...
        menu.append(
          new MenuItem({
            label: "Inspect Element",
            click: () =>
              adminWindow!.webContents.inspectElement(params.x, params.y),
          }),
        );
        menu.popup();
      });
      ...
      adminWindow.on("close", () =>
        saveSecondaryWindowState("admin", adminWindow!),
      );
  ...
  createTypedHandler(
    systemContracts.openPlaygroundWindow,
    async (event, { theme, themeIntensity }) => {
      // If window already exists, focus it
      if (playgroundWindow && !playgroundWindow.isDestroyed()) {
      ...
      const savedPlayground = getSavedWindowBounds("playground", {
        width: 900,
        height: 700,
      });
      ...
      playgroundWindow.webContents.on("context-menu", (_e, params) => {
        ...
        menu.append(
          new MenuItem({
            label: "Inspect Element",
            click: () =>
              playgroundWindow!.webContents.inspectElement(params.x, params.y),
          }),
        );
        menu.popup();
      });
      ...
      playgroundWindow.on("close", () =>
        saveSecondaryWindowState("playground", playgroundWindow!),
      );
  ...
  createTypedHandler(
    systemContracts.openDocsWindow,
    async (event, { theme, themeIntensity }) => {
      // If window already exists, focus it
      if (docsWindow && !docsWindow.isDestroyed()) {
        docsWindow.focus();
      ...
      const savedDocs = getSavedWindowBounds("docs", {
        width: 1000,
        height: 700,
      });
      ...
      docsWindow.webContents.on("context-menu", (_e, params) => {
        ...
        menu.append(
          new MenuItem({
            label: "Inspect Element",
            click: () =>
              docsWindow!.webContents.inspectElement(params.x, params.y),
          }),
        );
        menu.popup();
      });
      ...
      docsWindow.on("close", () =>
        saveSecondaryWindowState("docs", docsWindow!),
      );
  ...
  createTypedHandler(
    systemContracts.openReleaseNotesWindow,
    async (event, { theme, themeIntensity }) => {
      // If window already exists, focus it
      ...
      const savedDocs = getSavedWindowBounds("release-notes", {
        width: 1000,
        height: 700,
      });
      ...
      releaseNotesWindow.webContents.on("context-menu", (_e, params) => {
        ...
        menu.append(
          new MenuItem({
            label: "Inspect Element",
            click: () =>
              releaseNotesWindow!.webContents.inspectElement(
                params.x,
                params.y,
              ),
          }),
      ...
      releaseNotesWindow.on("close", () =>
        saveSecondaryWindowState("release-notes", releaseNotesWindow!),
      );
