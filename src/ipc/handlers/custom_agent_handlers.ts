  ...
  createTypedHandler(
    ...
    async (_, params, context) => {
      ...
      if (isDefaultBase === 1) {
        ...
        const resolvedBaseAgent =
          baseAgent !== undefined ? baseAgent : existing.baseAgent;

        const allAgents = await db
          .select()
          .from(remoteSchema.customAgents)
          .where(eq(remoteSchema.customAgents.userId, context.userId));
        const targetUltimateBase = getUltimateBaseAgent(
          resolvedBaseAgent,
          allAgents,
        );
