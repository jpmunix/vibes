...
export function registerOpenCodeDiagnosticHandlers() {
      ...
      if (openRouterSettings?.keys?.length > 0) {
        ...
        if (selectedKey) {
          ...
          try {
            ...
            const decryptedKey =
              secret.encryptionType === "plaintext"
                ? secret.value
                : decrypt(secret);
            keyDecryptionOk = decryptedKey?.startsWith("sk-");
            logger.info(
              `[OpenCode Diagnostic] Key "${selectedAlias}" decrypted successfully: ${keyDecryptionOk}`,
            );
          } catch (e) {
  ...
  ipcMain.handle(
    "opencode:test-run",
    async (_event, params?: { appPath?: string }) => {
  ...
  ipcMain.handle("opencode:extract-env", async () => {
    ...
    if (openRouterSettings?.keys?.length > 0) {
      ...
      if (selectedKey?.key) {
        try {
          const decrypted =
            selectedKey.key.encryptionType === "plaintext"
              ? selectedKey.key.value
              : decrypt(selectedKey.key);
          env.OPENROUTER_API_KEY = decrypted;
        } catch (e) {
          logger.error(
            "[OpenCode Diagnostic] Failed to decrypt OpenRouter key",
          );
        }
      }
    }
  ...
  logger.info(
    "[OpenCode Diagnostic] Handlers registered: opencode:health-check, opencode:test-run, opencode:extract-env",
  );
}
