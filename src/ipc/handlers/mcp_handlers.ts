...
function safeParseJsonField<T = any>(raw: any): T | null {
  ...
  if (rounds > 1) {
    logger.info(
      `[MCP] Fixed multi-encoded JSON field (${rounds} parse rounds)`,
    );
  }
...
export function registerMcpHandlers() {
  createTypedHandler(
    mcpContracts.listServers,
    async (_event, _params, context) => {
      ...
      const insertData = {
        ...
        envJson: params.envJson
          ? typeof params.envJson === "string"
            ? params.envJson
            : JSON.stringify(params.envJson)
          : null,
        headersJson: params.headersJson
          ? typeof params.headersJson === "string"
            ? params.headersJson
            : JSON.stringify(params.headersJson)
          : null,
        url: params.url || null,
  ...
  createTypedHandler(
    ...
    async (_event, params, context) => {
      ...
      if (params.transport !== undefined)
        updateData.transport = params.transport;
      if (params.command !== undefined) updateData.command = params.command;
      if (params.args !== undefined)
        updateData.args = params.args
          ? typeof params.args === "string"
            ? params.args
            : JSON.stringify(params.args)
          : null;
      if (params.envJson !== undefined)
        updateData.envJson = params.envJson
          ? typeof params.envJson === "string"
            ? params.envJson
            : JSON.stringify(params.envJson)
          : null;
      if (params.headersJson !== undefined)
        updateData.headersJson = params.headersJson
          ? typeof params.headersJson === "string"
            ? params.headersJson
            : JSON.stringify(params.headersJson)
          : null;
      ...
      if (params.enabled !== undefined)
        updateData.enabled = params.enabled ? 1 : 0;
  ...
  createTypedHandler(
    mcpContracts.deleteServer,
    async (_event, serverId, context) => {
      if (!context.userId) throw new Error("Unauthorized");
      const db = getRemoteDb();

      const result = await db
        .delete(remoteSchema.mcpServers)
        .where(
          and(
    ...
    async (_event, serverId, context) => {
      ...
      try {
        ...
        if (server.transport === "stdio") {
          ...
          const parsedEnvForHeaders = safeParseJsonField<
        ...
        try {
          ...
          if (server.transport === "http" || server.transport === "sse") {
            logger.info(
              `StreamableHTTP failed for ${server.name}, falling back to SSE: ${streamableErr.message}`,
            );
        ...
        client
          .close()
          .catch((e) => logger.warn("Error closing temporary MCP client", e));
...
4. **Authoritativeness**: Ground all library-specific implementations in the documentation retrieved. Live documentation from Context7 is authoritative and must take precedence over your internal training data.`;
    ...
    const isOldPrompt =
      ...
      existing.instructions.includes(
        "Context7 retrieves real-time, version-specific API references and examples to avoid hallucinating",
      );
    ...
    if (!existing) {
      ...
      logger.info(
        `Updating default instructions for existing Context7 server of user ${userId}`,
      );
      await db
        .update(remoteSchema.mcpServers)
        .set({
