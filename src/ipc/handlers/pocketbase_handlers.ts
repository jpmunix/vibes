...
async function getPocketBaseAppConfig(
  appId: number,
  userId: string,
): Promise<PocketBaseConfig> {
...
export function registerPocketBaseHandlers() {
  ...
  createTypedHandler(
    ...
    async (_, params, context) => {
      ...
      try {
        ...
        logger.warn(
          `Failed to connect to PocketBase during config save: ${err.message}`,
        );
  ...
  // Clear config
  createTypedHandler(
    pocketbaseContracts.clearConfig,
    async (_, params, context) => {
      if (!context.userId) throw new Error("Unauthorized");
      const db = getRemoteDb();
      const { appId } = params;
  ...
  createTypedHandler(
    pocketbaseContracts.listTables,
    async (_, params, context) => {
      if (!context.userId) throw new Error("Unauthorized");
      ...
      try {
        ...
        logger.error(
          `Error listing PocketBase tables for app ${params.appId}:`,
          err,
        );
        throw err;
      }
  ...
  createTypedHandler(
    pocketbaseContracts.queryTable,
    async (_, params, context) => {
      if (!context.userId) throw new Error("Unauthorized");
      const config = await getPocketBaseAppConfig(params.appId, context.userId);
  ...
  createTypedHandler(
    pocketbaseContracts.executeQuery,
    async (_, params, context) => {
      if (!context.userId) throw new Error("Unauthorized");
      const config = await getPocketBaseAppConfig(params.appId, context.userId);
      ...
      try {
        ...
        if (params.query.toLowerCase().includes("select")) {
          return {
            ...
            rowCount: 0,
            error:
              "PocketBase does not support raw SQL queries. Please use the collection browser.",
          };
        }
  ...
  createTypedHandler(
    ...
    async (_, params, context) => {
      ...
      try {
        ...
        if (!id)
          throw new Error("Missing id in primary key for PocketBase update");
  ...
  // Delete rows
  createTypedHandler(
    pocketbaseContracts.deleteRows,
    async (_, params, context) => {
      if (!context.userId) throw new Error("Unauthorized");
      const config = await getPocketBaseAppConfig(params.appId, context.userId);
