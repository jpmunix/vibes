...
export async function uploadChatAttachment(
  ...
  if (!response.ok) {
    ...
    logger.error(
      `Bunny chat-attachment upload failed: ${response.status}`,
      errorText,
    );
...
export function registerBunnyHandlers() {
    ...
    async (_, params, context) => {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            `Error al subir a Bunny Storage: ${response.statusText}`,
          );
        }
  ...
  async function getBunnyAppConfig(
    appId: number,
    userId: string,
  ): Promise<BunnyConfig> {
    const db = getRemoteDb();
  ...
  async function getLibSqlClient(
    appId: number,
    userId: string,
  ): Promise<{ client: Client; dbName: string }> {
      ...
      try {
        ...
        const tables = await Promise.all(
          tableNames.map(async (name) => {
            ...
            // Get column info
            const infoResult = await client.execute(
  ...
  createTypedHandler(
    ...
    async (_, { appId, table, data }, context) => {
      ...
      try {
        ...
        return {
          success: true,
          row: result.rows[0] ? { ...result.rows[0] } : undefined,
        };
      } finally {
        client.close();
      }
