...
function getDateGroup(dateStr: string, now: Date = new Date()): string {
  ...
  const diffDays = Math.floor(
    (today.getTime() - commitDay.getTime()) / 86400000,
  );
      ...
      <pre className="typo-mono-xs leading-5 min-w-max">
        {lines.map((line, i) => {
          ...
          if (line.startsWith("+") && !line.startsWith("+++")) {
            bg = "bg-[#0d4a1f]";
            fg = "text-green-300";
            lineNumFg = "text-green-700";
          } else if (line.startsWith("-") && !line.startsWith("---")) {
            bg = "bg-[#4a0d0d]";
            fg = "text-red-300";
            lineNumFg = "text-red-700";
          } else if (line.startsWith("@@")) {
            bg = "bg-blue-900/20";
            fg = "text-blue-400";
            lineNumFg = "text-blue-700";
          } else if (
            ...
            lineNumFg = "text-muted-foreground/15";
          }
          ...
          return (
            <div key={i} className={cn("flex min-h-[20px]", bg)}>
              <span
                className={cn(
                  "w-11 shrink-0 text-right pr-2.5 border-r border-white/5 select-none",
                  lineNumFg,
                  "text-xs leading-5",
                )}
              >
              ...
              <span className={cn("px-3 flex-1 whitespace-pre", fg)}>
                {line || " "}
              </span>
            </div>
          );
    ...
    return (
      <div className="flex flex-col items-center justify-center py-16 px-4 text-center">
        ...
        <p className="text-xs text-muted-foreground/60 mt-1">
          Los commits aparecerán aquí una vez que hagas cambios
        </p>
      </div>
    );
  }
                ...
                <div className="px-4 py-3 border-b border-border/50 space-y-2">
                  ...
                  <div className="flex flex-wrap items-center gap-2.5 text-xs">
                    <Tooltip>
                      ...
                      <TooltipContent>
                        {formatFullDate(commitDetail.date)}
                      </TooltipContent>
                    </Tooltip>
