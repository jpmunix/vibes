...
export function FirebaseConnector({
    ...
    const timer = setTimeout(() => {
      if (
        ...
        const projectStillExists = projects.some(
          (p) => p.projectId === app.firebaseProjectId,
        );
        if (!projectStillExists) {
          unsetAppProject(appId).then(() => {
            toast.error(
              "El proyecto que estaba conectado ya no existe en Firebase.",
            );
            refreshApp();
          });
        }
  ...
  const handleProjectSelect = async (
    ...
    try {
      ...
      const configWithName = {
        ...config,
        webAppDisplayName: displayName || null,
      };
      await setAppProject({
        appId,
        projectId,
  ...
  const handleCreateProject = async () => {
    ...
    try {
      ...
      const success = await handleProjectSelect(
        project.projectId,
        undefined,
        project.displayName,
      );
  ...
  if (isConnected && app?.firebaseProjectId) {
    ...
    const handleSelectWebAppFromManage = async (
      webAppId: string,
      displayName?: string,
    ) => {
      setIsConnecting(true);
    ...
    const handleCreateWebAppFromManage = async () => {
      ...
      setIsInternalProcessing(true);
      try {
        const result = await createWebApp(
          app.firebaseProjectId!,
          newWebAppName,
        );
          ...
          <div className="space-y-3 p-3 border rounded-lg bg-muted/20">
            <div className="flex items-center justify-between">
              ...
              <Button
                variant="ghost"
                size="sm"
                onClick={() => setShowManageWebApps(false)}
                disabled={isWorking}
            ...
            {isLoadingWebApps ? (
              ...
              <div className="space-y-4 p-3 border rounded-lg bg-muted/20">
                <div className="space-y-2">
                  <Label htmlFor="web-app-name-manage">
                    Nombre de la nueva App Web
                  </Label>
                  <Input
                ...
                <div className="flex gap-2">
                  <Button
                    variant="outline"
                    className="flex-1"
                    onClick={handleCreateWebAppFromManage}
                    disabled={isWorking}
                  >
                    {isInternalProcessing ? (
                  ...
                  <Button
                    variant="outline"
                    onClick={() => setShowCreateWebAppForm(false)}
                    disabled={isWorking}
                  >
                    Cancelar
                  </Button>
                ...
                <div className="grid gap-2">
                  {webApps.length === 0 ? (
                    <p className="text-xs text-muted-foreground italic py-2">
                      No hay apps web en este proyecto.
                    </p>
                  ) : (
                    webApps.map((wa) => {
                      const isCurrentApp =
    ...
    return (
      <Card className="mt-1">
        <CardHeader className="pb-2">
          ...
          <CardDescription>
            {showCreateForm
              ? "Carga un nuevo proyecto de Firebase"
              : "Selecciona un proyecto para esta app."}
          </CardDescription>
        </CardHeader>
  ...
  return (
    <Card className="mt-1 border-dashed">
      <CardHeader className="pb-2">
        ...
        <CardDescription>
          Conecta tu cuenta de Google para gestionar tus proyectos.
        </CardDescription>
      </CardHeader>
      <CardContent>{content}</CardContent>
    </Card>
