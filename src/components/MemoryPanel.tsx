  ...
  const loadMemories = useCallback(async () => {
    ...
    try {
      ...
      const filtered = raw.filter(
        (m) => m.type === "preference" && m.source === "manual",
      );
                ...
                <div className="flex items-center justify-between gap-2">
                  <div className="flex items-center gap-1.5 flex-wrap min-w-0">
                    ...
                    <span className="flex items-center gap-1 typo-micro text-muted-foreground/70">
                      <span
                        className={`inline-block w-1.5 h-1.5 rounded-full ${impColor}`}
                      />
                      {imp} · {impLabel}
                    </span>
                  </div>
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-2">
              <label className="typo-label flex justify-between">
                ...
                <span
                  className={`typo-caption font-semibold ${
                    createForm.importance >= 90
                      ? "text-rose-500"
                      : createForm.importance >= 70
                        ? "text-amber-500"
                        : "text-blue-500"
                  }`}
                >
              ...
              <div className="rounded-lg border border-border bg-muted/30 p-3 space-y-1">
                <p className="typo-micro font-semibold text-muted-foreground/80 uppercase tracking-wider">
                  Guía de importancia
                </p>
                <div className="grid gap-0.5 typo-micro text-muted-foreground">
                  <span>
                    ...
                    90–100 · Crítico: stack principal, convenciones globales del
                    proyecto
                  </span>
                  <span>
                    <span className="inline-block w-2 h-2 rounded-full bg-amber-500 mr-1.5" />
                    70–85 · Alto: arquitectura de módulos, librerías clave,
                    flujos de auth
                  </span>
                  <span>
                    <span className="inline-block w-2 h-2 rounded-full bg-blue-500 mr-1.5" />
                    50–65 · Medio: gotchas técnicos, preferencias de formato,
                    rarezas de APIs
                  </span>
                </div>
              </div>
                ...
                <span
                  className={`typo-caption font-semibold ${
                    editForm.importance >= 90
                      ? "text-rose-500"
                      : editForm.importance >= 70
                        ? "text-amber-500"
                        : editForm.importance >= 50
                          ? "text-blue-500"
