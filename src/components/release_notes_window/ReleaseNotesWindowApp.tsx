/**
 * ReleaseNotesWindowApp — Dedicated release notes viewer window.
 *
 * Layout:
 *   TitleBar (draggable, with window controls)
 *   ├── ReleaseNotesSidebar (tree navigation, resizable)
 *   └── ReleaseNotesContent (article rendered from markdown)
 */
...
function ReleaseNotesWindowContent() {
  ...
  const handleNavigate = useCallback(
    (relativePath: string, anchor?: string, query?: string) => {
  ...
  useEffect(() => {
    if (settings) {
      const root = document.documentElement;
      if (settings.fontScaleUI !== undefined)
        root.style.setProperty("--scale-ui", settings.fontScaleUI.toString());
      if (settings.fontScaleSidebar !== undefined)
        root.style.setProperty(
          "--scale-sidebar",
          settings.fontScaleSidebar.toString(),
        );
      if (settings.fontScaleChat !== undefined)
        root.style.setProperty(
          "--scale-chat",
          settings.fontScaleChat.toString(),
        );
    }
  }, [
  ...
  const { data: docTree, isLoading: treeLoading } = useQuery({
    queryKey: ["docTree", "release-notes"],
    queryFn: () => ipc.system.getDocTree({ baseDir: "release-notes" }),
  });
  ...
  // Restore sidebar width from preferences (default to max)
  useEffect(() => {
    ipc.misc
      .getPreference({ key: "releaseNotes.sidebarWidth" })
      .then((raw) => {
        if (raw) {
          const w = Number(raw);
      ...
      .catch(() => {});
    // Reset any old narrow preference to max on first use of the new default
    ipc.misc
      .setPreference({ key: "releaseNotes.sidebarWidth", value: "400" })
      .catch(() => {});
  }, []);
...
export function ReleaseNotesWindowApp() {
  return (
