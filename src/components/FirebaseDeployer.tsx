...
export function FirebaseDeployer({ appId, app }: FirebaseDeployerProps) {
  ...
  const openConsole = () => {
    ipc.system.openExternalUrl(
      `https://console.firebase.google.com/project/${app.firebaseProjectId}/hosting/main`,
    );
  };
      ...
      <div className="space-y-4 pt-2">
        <div className="flex items-center justify-between p-3 rounded-lg border border-gray-200 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-900/20">
          ...
          <Button
            ...
            size="icon"
            onClick={openConsole}
            title="Abrir consola de Firebase"
          >
        ...
        {isDeploying && (
          <p className="typo-caption text-muted-foreground italic">
            Puedes ver el progreso en la pestaña de mensajes del sistema
            (Consola).
          </p>
        )}
      </div>
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription className="space-y-3">
              <p>
                Para poder desplegar tu aplicación, necesitas habilitar el
                servicio de **Firebase Hosting** en tu proyecto de Google Cloud.
              </p>
              <p className="typo-caption bg-gray-100 dark:bg-gray-800 p-2 rounded border">
                Proyecto:{" "}
                <span className="typo-mono">{app.firebaseProjectId}</span>
              </p>
              <p>
                Haz clic en el siguiente botón para abrir la consola de Google y
                activar el servicio. Una vez activado, espera unos 30 segundos y
          ...
          <DialogFooter className="mt-4">
            <Button
              variant="outline"
              onClick={() => setShowEnableApiDialog(false)}
            >
              Cancelar
            </Button>
            <Button
