        ...
        {operations.map((op: any, i: number) => (
          ...
          >
            ...
            <div className="flex items-center gap-3 mt-2 typo-micro text-muted-foreground">
              {op.key && (
                <span className="typo-mono-xs bg-muted/50 px-1.5 py-0.5 rounded">
                  key:{op.key}
                </span>
              )}
              <span>imp:{normalizeImp(op.importance)}</span>
              {op.action && (
                <span className="text-primary/70">{op.action}</span>
              )}
            </div>
          </div>
        ))}
        ...
        <DropdownMenuContent align="start" className="min-w-[180px]">
          ...
          {presets.map((set) => {
            ...
            return (
              <DropdownMenuItem
                key={set.name}
                className={cn("cursor-pointer py-2", alreadyIn && "opacity-40")}
                disabled={alreadyIn}
  ...
  return (
    <div
      className={cn(
        ...
        isTimeout && "border-destructive/30 bg-destructive/5",
      )}
    >
      <div
        className={cn(
      ...
      >
        ...
        {rank != null && (
          <span
            className={`playground-rank ${rank === 1 ? "gold" : rank === 2 ? "silver" : rank === 3 ? "bronze" : ""}`}
          >
            #{rank}
          </span>
        )}
        <div className="flex-1 min-w-0">
          <h3
            className={cn(
              "typo-label !text-sm font-semibold truncate",
          ...
          {hasPricing && (
            <div className="typo-caption text-muted-foreground/50 truncate flex items-center gap-1.5 mt-0.5">
              <span>
                In{" "}
                <span className="tabular-nums">{fmtPricePM(pricingInput)}</span>
              </span>
              <span className="opacity-40">·</span>
              <span>
                Out{" "}
                <span className="tabular-nums">
                  {fmtPricePM(pricingOutput)}
                </span>
              </span>
            </div>
          )}
        ...
        <div className="flex items-center gap-3 shrink-0">
          ...
          <span
            className={cn(
              "flex items-center gap-1 typo-caption",
              isTimeout ? "text-destructive" : "text-muted-foreground",
            )}
          >
          ...
          {isTimeout && <AlertCircle size={14} className="text-destructive" />}
          {onRetry && (
            <button
              type="button"
...
function PlaygroundPanel() {
  ...
  const [pendingDeletePreset, setPendingDeletePreset] = useState<string | null>(
    null,
  );
  const [pendingRenamePreset, setPendingRenamePreset] = useState<string | null>(
    null,
  );
  ...
  // Prompt presets
  const [activePromptPreset, setActivePromptPreset] = useState<string | null>(
    null,
  );
  ...
  const [pendingDeletePromptPreset, setPendingDeletePromptPreset] = useState<
    string | null
  >(null);
  const [pendingRenamePromptPreset, setPendingRenamePromptPreset] = useState<
    string | null
  >(null);
    ...
    if (
      ...
      presets.migrateFromSettings(legacy).then((migrated) => {
        if (migrated) {
          ...
          updateSettings(rest);
          console.log(
            "[Playground] Migrated model presets to DB, cleaned settings",
          );
        }
      });
    }
  ...
  const handleToggleModel = useCallback((apiName: string) => {
    setDisabledModels((prev) => {
      const s = new Set(prev);
      if (s.has(apiName)) s.delete(apiName);
      else s.add(apiName);
      return s;
    });
  }, []);
  ...
  const activeModels = useMemo(
    () => selectedModels.filter((m) => !disabledModels.has(m)),
  ...
  const displayResults = useMemo(() => {
    ...
    return [...results].sort((a, b) => {
      switch (sortMode) {
        ...
        case "price-in-asc":
          return (
            (priceMap.get(a.modelApiName)?.inp ?? 0) -
            (priceMap.get(b.modelApiName)?.inp ?? 0)
          );
        case "price-in-desc":
          return (
            (priceMap.get(b.modelApiName)?.inp ?? 0) -
        ...
        case "price-out-asc":
          return (
            (priceMap.get(a.modelApiName)?.out ?? 0) -
            (priceMap.get(b.modelApiName)?.out ?? 0)
          );
        case "price-out-desc":
          return (
            (priceMap.get(b.modelApiName)?.out ?? 0) -
  ...
  const sortOptions: {
    ...
    {
      key: "price-in-asc",
      label: "Entrada más barato",
      icon: <DollarSign size={13} />,
    },
    {
      key: "price-in-desc",
      label: "Entrada más caro",
      icon: <DollarSign size={13} />,
    },
    {
      key: "price-out-asc",
      label: "Salida más barato",
      icon: <DollarSign size={13} />,
    },
    {
      key: "price-out-desc",
      label: "Salida más caro",
      icon: <DollarSign size={13} />,
    },
  ];
  ...
  const handleAnalyze = useCallback(async () => {
    ...
    try {
      const response = await ipc.misc.playgroundAnalyze({
        model: analysisModel,
        originalPrompt: prompt,
        results: completedResults.map((r) => ({
          modelApiName: r.modelApiName,
          modelDisplayName:
            modelDisplayNameMap.get(r.modelApiName) || r.modelApiName,
          text: r.text,
      ...
      try {
        ...
        parsed = {
          error: "No se pudo parsear la respuesta del analista.",
          raw: response.text,
        };
      }
  ...
  }, [isAnalyzing, runFinished, results, analysisModel, prompt]);

  // Retry a single model
  const handleRetryModel = useCallback(
    async (modelApiName: string) => {
      ...
      try {
        ...
        setResults((prev) =>
          prev.map((r) => (r.modelApiName === modelApiName ? newResult : r)),
        );
      ...
      try {
        ...
        if (cancelledRef.current || skipCurrentRef.current) {
          if (skipCurrentRef.current) {
            ...
            setModelTimes((prev) =>
              new Map(prev).set(modelApiName, durationMs),
            );
            continue;
          }
          break;
  ...
  const handleMergePreset = useCallback(
    (name: string) => {
      const preset = modelSets.find((s) => s.name === name);
      if (!preset) return;
  ...
  const handleExportMarkdown = useCallback(() => {
    ...
    toExport.forEach((r, idx) => {
      ...
      if (r.inputTokens != null)
        lines.push(`| Tokens entrada | ${r.inputTokens} |`);
      if (r.outputTokens != null)
        lines.push(`| Tokens salida | ${r.outputTokens} |`);
    ...
    <div ref={resultsRef} className="playground-scroll-root">
      <style>{`
                ...
                .playground-analyze-btn {
                    ...
                    border: none;
                    border-radius: 8px;
                    background: linear-gradient(135deg, oklch(from var(--primary) l c h), oklch(from var(--primary) calc(l - 0.08) calc(c + 0.02) calc(h + 15)));
                    color: var(--primary-foreground);
                    font-size: 12px;
                    font-weight: 600;
                    cursor: pointer;
                ...
                .playground-analyze-btn:hover:not(:disabled) {
                    opacity: 0.92;
                ...
                .score-fill.speed { background: oklch(0.75 0.15 85); }
                .score-fill.overall { background: var(--primary); }
            `}</style>
      ...
      <div className="playground-input-area">
          ...
          {orderedChips.map((apiName) => {
            ...
            return (
              <ModelChipWithPresets
                ...
                onAddToPreset={(presetName) =>
                  handleAddModelToPreset(apiName, presetName)
                }
              />
            );
          })}
                  ...
                  >
                    <div className="flex flex-col min-w-0">
                      ...
                      <span className="typo-micro text-muted-foreground truncate">
                        {set.models.length} modelo
                        {set.models.length !== 1 ? "s" : ""}
                      </span>
                    </div>
                ...
                <AlertDialogAction
                  ...
                  onClick={() => {
                    if (pendingDeletePreset)
                      handleDeletePreset(pendingDeletePreset);
            ...
            <AlertDialogContent>
              ...
              <AlertDialogFooter>
                ...
                <AlertDialogAction
                  disabled={
                    !renameValue.trim() ||
                    renameValue.trim() === pendingRenamePreset
                  }
                  onClick={() => {
                    if (pendingRenamePreset)
                      handleRenamePreset(pendingRenamePreset, renameValue);
                  }}
                >
                  Renombrar
        ...
        <div className="playground-toolbar">
          ...
          <button
            ...
            title={
              autoCollapse
                ? "Desactivar auto-colapso"
                : "Activar auto-colapso: colapsar resultado anterior al terminar uno nuevo"
            }
          ...
          <button
            ...
            style={{
              ...
              border: "1px solid var(--border)",
              opacity:
                selectedModels.length === 0 && results.length === 0 && !prompt
                  ? 0.3
                  : 1,
            }}
          >
          ...
          <button
            ...
            className={cn(
              "playground-send-btn",
              isRunning && "playground-send-btn--cancel",
            )}
            onClick={isRunning ? handleCancel : handleSubmit}
            disabled={
              !isRunning && (!prompt.trim() || activeModels.length === 0)
            }
          >
            {isRunning ? (
              <>
                ...
                <div className="playground-analysis-results">
                  {analysisResult.summary && (
                    <div className="playground-analysis-summary">
                      ...
                      />
                      <p className="typo-body text-sm leading-relaxed">
                        {analysisResult.summary}
                      </p>
                    </div>
                  )}
                  <div className="playground-analysis-winners">
                    {analysisResult.bestQualityTime && (
                      <div className="playground-winner-card ratio">
                        <div className="winner-header">
                          <div className="winner-icon ratio">
                            <Zap size={16} />
                          </div>
                          <div>
                            <h4 className="typo-label text-xs uppercase tracking-wider opacity-60">
                              Mejor Calidad / Tiempo
                            </h4>
                            <p className="typo-label text-sm font-bold mt-0.5">
                              {analysisResult.bestQualityTime.modelDisplayName}
                            </p>
                          </div>
                          <div className="winner-score">
                            {analysisResult.bestQualityTime.score}
                          </div>
                        </div>
                        <p className="typo-body text-xs leading-relaxed text-muted-foreground mt-3">
                          {analysisResult.bestQualityTime.justification}
                        </p>
                      </div>
                    )}
                    {analysisResult.bestQualityOnly && (
                      <div className="playground-winner-card quality">
                        <div className="winner-header">
                          <div className="winner-icon quality">
                            <Trophy size={16} />
                          </div>
                          <div>
                            <h4 className="typo-label text-xs uppercase tracking-wider opacity-60">
                            ...
                            <p className="typo-label text-sm font-bold mt-0.5">
                              {analysisResult.bestQualityOnly.modelDisplayName}
                            </p>
                          </div>
                          <div className="winner-score">
                            {analysisResult.bestQualityOnly.score}
                          </div>
                        </div>
                        <p className="typo-body text-xs leading-relaxed text-muted-foreground mt-3">
                          {analysisResult.bestQualityOnly.justification}
                        </p>
                      </div>
                    )}
                  </div>
                  {analysisResult.rankings &&
                                ...
                                <div className="score-cell">
                                  <div className="score-bar">
                                    <div
                                      ...
                                      style={{ width: `${r.speedScore || 0}%` }}
                                    />
                                  </div>
                                  <span>{r.speedScore}</span>
                                </div>
                                <div className="score-cell">
                                  <div className="score-bar">
                                    <div
        ...
        >
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <DropdownMenu
              open={promptPresetMenuOpen}
              onOpenChange={setPromptPresetMenuOpen}
            >
              <DropdownMenuTrigger asChild>
                <button
                  type="button"
      ...
      {/* Save prompt dialog */}
      <AlertDialog
        open={promptSaveDialogOpen}
        onOpenChange={setPromptSaveDialogOpen}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
      ...
      <AlertDialog
        open={!!pendingRenamePromptPreset}
        onOpenChange={(open) => {
          if (!open) setPendingRenamePromptPreset(null);
        }}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
          ...
          <input
            ...
            onKeyDown={(e) => {
              if (
                e.key === "Enter" &&
                promptRenameValue.trim() &&
                pendingRenamePromptPreset
              ) {
                presets.renamePromptPreset(
                  pendingRenamePromptPreset,
                  promptRenameValue.trim(),
                );
                if (activePromptPreset === pendingRenamePromptPreset)
                  setActivePromptPreset(promptRenameValue.trim());
      ...
      <AlertDialog
        open={!!pendingDeletePromptPreset}
        onOpenChange={(open) => {
          if (!open) setPendingDeletePromptPreset(null);
        }}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogAction
              ...
              onClick={() => {
                if (pendingDeletePromptPreset) {
                  ...
                  if (activePromptPreset === pendingDeletePromptPreset)
                    setActivePromptPreset(null);
      ...
      <div className="playground-results">
        {displayResults.map((result, i) => {
          ...
          const modelInfo = allModels.find(
            (m) => m.apiName === result.modelApiName,
          );
          return (
            <ResultCard
              key={cardKey}
              ...
              onRetry={
                runFinished && !isRunning
                  ? () => handleRetryModel(result.modelApiName)
                  : undefined
              }
              ...
              exportChecked={
                runFinished
                  ? exportSelection.has(result.modelApiName)
                  : undefined
              }
              onExportToggle={
                runFinished
                  ? () => {
                      setExportSelection((prev) => {
                        ...
                        if (next.has(result.modelApiName))
                          next.delete(result.modelApiName);
        ...
        {runFinished && exportSelection.size > 0 && (
          <div className="flex items-center justify-between px-4 py-2.5 rounded-xl border border-primary/20 bg-primary/5">
            <span className="typo-caption text-primary">
              {exportSelection.size} resultado
              {exportSelection.size !== 1 ? "s" : ""} seleccionado
              {exportSelection.size !== 1 ? "s" : ""}
            </span>
            <div className="flex items-center gap-2">
              <button
                type="button"
        ...
        {isRunning &&
          currentModelIndex >= 0 &&
          currentModelIndex < activeModels.length && (
        ...
        {!isRunning && results.length === 0 && (
          <div className="playground-empty">
            ...
            <span className="text-sm">
              Selecciona modelos y envía un prompt para comparar
            </span>
          </div>
        )}
      </div>
...
function PlaygroundWindowContent() {
  ...
  useEffect(() => {
    document.title = "Playground de modelos";
  }, []);
