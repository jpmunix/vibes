...
export function DocsSidebar({
  ...
  const { data: searchResults, isFetching: isSearching } = useQuery({
    ...
    queryFn: () => ipc.system.searchDocs({ query: debouncedQuery }),
    ...
    return (
      <div className="flex-1 p-4 space-y-3">
        {[...Array(6)].map((_, i) => (
          <div
            key={i}
            className="h-6 rounded-md bg-muted/50 animate-pulse"
            style={{
              width: `${60 + (i % 3) * 15}%`,
...
function HighlightedSnippet({
  ...
  return (
    <>
      {before}
      <span className="text-primary font-semibold bg-primary/10 rounded-sm px-0.5">
        {match}
      </span>
      {after}
    </>
