...
function ChatWindowContent({
  ...
  useEffect(() => {
    if (settings) {
      ...
      if (settings.fontScaleBubbleWidth !== undefined)
        root.style.setProperty(
          "--bubble-width",
        ...
        if (pending) {
          streamMessage({
            ...
            attachments: pending.attachments?.map((a) => ({
              file: new File(
                [
                  Uint8Array.from(atob(a.data.split(",")[1] || a.data), (c) =>
                    c.charCodeAt(0),
                  ),
                ],
                a.name,
  ...
  useEffect(() => {
    const unsubscribe = ipc.events.misc.onChatStreamEnd(({ chatId }) => {
      ...
      if (!serverReady) {
        ...
        const isCodeProducingMode =
          currentMode !== "plan" && currentMode !== "ask";
  ...
  return (
    <div className="flex flex-col h-screen w-full bg-background">
      ...
      <div className="flex flex-1 min-h-0">
        <PanelGroup
          autoSaveId={`chat-window-${appId}-${chatPosition}`}
...
function createChatWindowRouter(
  appId: number,
  chatId?: number,
  hasPendingPrompt?: boolean,
  initialChatMode?: string,
...
export function ChatWindowApp({
  ...
  const [chatRouter] = useState(() =>
    createChatWindowRouter(appId, chatId, hasPendingPrompt, initialChatMode),
  );
