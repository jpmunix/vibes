...
const PROVIDER_BADGE_STYLES: Record<
  string,
  ...
  const { options, groups } = useMemo(() => {
    ...
    if (showProviderBadge && uniqueProviders > 1) {
      ...
      const buckets = new Map<
        string,
        { label: string; models: SelectorOption[] }
      >();
      ...
      for (const model of filteredModels) {
        ...
        if (!buckets.has(groupId)) {
          ...
          if (groupId === "ollama") groupLabel = "Ollama (local)";
            ...
            const name =
              getSourceProviderLabel(model) || groupId.replace("custom::", "");
            groupLabel = name;
      ...
      const orderedBuckets = [
        ...
        ...[...buckets.entries()].filter(
          ([k]) =>
            k !== "ollama" && k !== "openrouter" && !k.startsWith("custom::"),
        ),
      ];
  ...
  return (
    <>
      <UnifiedSelector
        ...
        customTriggerLabel={
          <span className="truncate flex-1 text-left">{getDisplayName()}</span>
        }
          ...
          if (model) {
            ...
            return (
              <div className="flex items-center gap-2 w-full">
                ...
                {badge && (
                  <span
                    className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-md shrink-0 ${badge.bg} ${badge.text}`}
                  >
                    {badge.label}
                  </span>
                )}
