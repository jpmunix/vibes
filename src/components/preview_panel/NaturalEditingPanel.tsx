...
export function NaturalEditingPanel({
  ...
  const iframeRefObj = useMemo(() => ({ current: iframeRef }), [iframeRef]);
  ...
  const sendStyleModification = useCallback(
    (styles: Record<string, any>) => {
      ...
      console.log(
        "[sendStyleModification] iframe disponible?",
        !!iframeRefObj.current?.contentWindow,
      );
      console.log(
        "[sendStyleModification] componente seleccionado?",
        !!selectedComponent,
      );
      ...
      setPendingChanges((prev) => {
        ...
        console.log(
          "[NaturalEditingPanel] Total pending changes:",
          updated.size,
        );
  ...
  const sendTextPreview = useCallback(
    (text: string) => {
      if (!selectedComponent || !iframeRefObj.current?.contentWindow) return;
      iframeRefObj.current.contentWindow.postMessage(
        {
  ...
  const typeConfig =
    ELEMENT_TYPE_CONFIG[elementType] || ELEMENT_TYPE_CONFIG.unknown;
...
function QuickEditChat({
  ...
  return (
    <div
      className={`nep-quick-chat${isExpanded ? " nep-quick-chat--expanded" : ""}`}
    >
      <div className="nep-quick-chat-header">
        ...
        >
          <span className="nep-quick-chat-title">
            Explica lo que quieres cambiar
          </span>
          {isExpanded && (
            <button
              className="nep-close-btn"
        ...
        {!isExpanded && (
          <span className="nep-quick-chat-subtitle">
            Para cambios pequeños al elemento seleccionado
          </span>
        )}
      </div>
      <div className="nep-quick-chat-content">
        {error && <div className="nep-quick-chat-error">{error}</div>}
              ...
              {PHASES.map((phase, i) => (
                ...
                >
                  {i < processingPhase ? (
                    ...
                    <Loader2
                      size={12}
                      className="nep-quick-chat-phase-spinner"
                    />
                  ) : (
