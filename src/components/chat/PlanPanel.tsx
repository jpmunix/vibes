...
export function parsePlanFromText(text: string): Plan | null {
  ...
  for (const line of lines) {
    ...
    if (
      ...
      objective = trimmed.replace(/^#\s*Objetivo[:\s]*/i, "").trim();
      continue;
    }

    // Stage header
    const stageMatch = trimmed.match(/^##\s+(Etapa\s+\d+[:\s]*)?(.+)/i);
    if (stageMatch) {
 ...
 */
export function planToPromptText(plan: Plan, onlyChecked: boolean): string {
  const parts: string[] = [];
  if (plan.objective) {
            ...
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter") {
                  onUpdateStage({
                    ...stage,
                    title: titleText.trim() || stage.title,
                  });
...
export function PlanPanel({ chatId }: { chatId?: number }) {
  ...
  const savePlan = useCallback(
    (newPlan: Plan) => {
      ...
      ipc.chat
        .savePlanData({ chatId, planData: newPlan })
        .then(() => {
  ...
  const toggleAll = () => {
    ...
    savePlan({
      ...
      stages: plan.stages.map((s) => ({
        ...s,
        tasks: s.tasks.map((t) =>
          t.isDeveloped ? t : { ...t, checked: newChecked },
        ),
      })),
    });
  };
  ...
  const handleDevelop = (onlySelected: boolean) => {
    ...
    savePlan({
      ...
      stages: plan.stages.map((s) => ({
        ...s,
        tasks: s.tasks.map((t) =>
          taskIdsToMark.has(t.id)
            ? { ...t, isDeveloped: true, checked: false }
            : t,
        ),
      })),
    });
