...
export const FilesChangedBar = React.memo(function FilesChangedBar({
  ...
  const label =
    files === 1 ? "1 archivo modificado" : `${files} archivos modificados`;
...
function InlineCommitPanel({
  ...
  const { uncommittedFiles, commit, push, isCommitting, isPushing } =
    useGitPanel(appId);
  ...
  const allCommitted =
    scopedBasenames.length > 0 &&
    matchedFiles.length === 0 &&
    uncommittedFiles.length >= 0;

  // Inject synthetic commit message into chat (same pattern as GitQuickCommit)
  const injectSyntheticCommitMessage = useCallback(
    (action: "commit" | "commit-push", message: string) => {
  ...
  const handleGenerateMessage = useCallback(() => {
    ...
    const filesToPass = matchedFiles.map((f) => ({
      path: f.path,
      status: f.status,
    }));
  ...
  return (
    <div className="px-4 pb-4 pt-2 animate-in slide-in-from-top-1 duration-150">
      ...
      <div className="flex items-center gap-2 text-xs text-muted-foreground/70 mb-3">
        <GitCommit size={13} />
        <span>
          {filesToStage.length} de {scopedBasenames.length} archivo
          {scopedBasenames.length !== 1 ? "s" : ""} pendiente
          {filesToStage.length !== 1 ? "s" : ""}
        </span>
      </div>
