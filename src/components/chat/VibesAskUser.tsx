...
export const VibesAskUser: React.FC<VibesAskUserProps> = ({
  children,
  node,
}) => {
        ...
        prev.filter(
          (p) =>
            !(
              ...
              p.questionIndex === pendingEntry.questionIndex
  ...
  return (
    ...
    >
        ...
        <div className="ml-auto flex items-center gap-1.5">
          {isPartOfGroup && (
            <span
              ...
              style={{ color: "var(--accent-teal-label)" }}
            >
              {(pendingEntry?.questionIndex ?? 0) + 1}/
              {pendingEntry?.totalQuestions}
            </span>
          )}
          <span
      ...
      <div className="px-4 py-3">
        {/* Question text */}
        <p className="text-[13px] leading-relaxed text-foreground/90 mb-1">
          {question}
        </p>
        ...
        <div className="mt-3 space-y-2">
                    ...
                    onMouseEnter={(e) => {
                      if (!isSelected) {
                        e.currentTarget.style.background =
                          "var(--accent-teal-option-hover-bg)";
                        e.currentTarget.style.borderColor =
                          "var(--accent-teal-option-hover-border)";
                      }
                    }}
                    onMouseLeave={(e) => {
                      if (!isSelected) {
                      ...
                      {isMultiple ? (
                        isSelected ? (
                          <CheckSquare
                            size={16}
                            style={{
                              color: "var(--accent-teal-selected-icon)",
                            }}
                          />
                        ) : (
                          <Square
                            size={16}
                            style={{
                              color: "var(--accent-teal-checkbox-inactive)",
                            }}
                          />
                        )
                      ) : isSelected ? (
                  ...
                  {useCustom ? (
                    isMultiple ? (
                      ...
                      <div
                        className="w-4 h-4 rounded-full flex items-center justify-center"
                        style={{
                          border: "2px solid var(--accent-teal-selected-icon)",
                        }}
                      >
                        <div
          ...
          {(useCustom || options.length === 0) && (
            <div className="pt-1">
              <input
                ...
                onFocus={(e) => {
                  e.currentTarget.style.borderColor =
                    "var(--accent-teal-input-focus-border)";
                  e.currentTarget.style.boxShadow =
                    "0 0 0 3px var(--accent-teal-input-focus-ring)";
                }}
                onBlur={(e) => {
                  e.currentTarget.style.borderColor =
                    "var(--accent-teal-input-border)";
          ...
          <div className="flex items-center justify-between pt-1">
            {isMultiple && selectedOptions.size > 0 && !useCustom && (
              <span
                className="text-[11px]"
                style={{ color: "var(--accent-teal-count-text)" }}
              >
                {selectedOptions.size} seleccionada
            ...
            <button
              ...
              style={{
                ...
                color: canSend
                  ? "var(--accent-teal-btn-text)"
                  : "var(--accent-teal-btn-disabled-text)",
                boxShadow: canSend
              ...
              onMouseEnter={(e) => {
                if (canSend) {
                  e.currentTarget.style.background =
                    "linear-gradient(135deg, var(--accent-teal-btn-hover-from), var(--accent-teal-btn-hover-to))";
                }
              }}
              onMouseLeave={(e) => {
                if (canSend) {
