...
const TOOL_LABELS: Record<string, { label: string; description: string }> = {
  file_editor: {
    ...
    description: "Modificar el contenido de archivos existentes en tu proyecto",
  },
  delete_file: {
    label: "Eliminar archivos",
    description: "Borrar archivos de tu proyecto de forma permanente",
  },
  rename_file: {
    label: "Renombrar archivos",
    description: "Cambiar el nombre o mover archivos dentro de tu proyecto",
  },
  add_dependency: {
    label: "Instalar dependencias",
    description: "Añadir paquetes o librerías al proyecto",
  },
  execute_sql: {
    label: "Ejecutar consultas SQL",
    description: "Ejecutar consultas en tu base de datos conectada",
  },
  explore_codebase: {
  ...
  run_command: {
    ...
    description: "Ejecutar comandos en la terminal de tu equipo",
  },
  start_process: {
    label: "Iniciar procesos",
    description: "Arrancar servicios o servidores en segundo plano",
  },
  stop_process: {
    label: "Detener procesos",
    description: "Parar servicios o servidores en ejecución",
  },
  list_processes: {
    label: "Listar procesos activos",
    description: "Consultar qué servicios o servidores están activos",
  },
  wait_for_http: {
    label: "Esperar disponibilidad web",
    description: "Esperar a que un servidor esté listo para responder",
  },
  get_supabase_project_info: {
    label: "Consultar info de Supabase",
    description: "Obtener información sobre tu proyecto de Supabase",
  },
  ...
  read_logs: {
    ...
    description: "Consultar los registros del sistema y la aplicación",
  },
  web_crawl: {
    label: "Navegar páginas web",
    description: "Acceder a páginas web para extraer información",
  },
  update_todos: {
    label: "Gestionar tareas",
    description: "Actualizar el progreso de las tareas que está realizando",
  },
  run_type_checks: {
    label: "Verificar errores de código",
    description: "Ejecutar verificación de tipos para detectar errores",
  },
  git_operations: {
    label: "Operaciones Git",
    description: "Commits, ramas y estado del repositorio",
  },
  ask_user: {
  ...
  write_file: {
    ...
    description: "Crear nuevos archivos en tu proyecto",
  },
  read_file: {
    label: "Leer archivos",
    description: "Leer el contenido de archivos de tu proyecto",
  },
  edit_file: {
    label: "Editar archivos (búsqueda)",
    description: "Buscar y reemplazar contenido en archivos",
  },
  search_replace: {
    label: "Buscar y reemplazar",
    description: "Buscar texto en archivos y reemplazarlo",
  },
  patch_file: {
    label: "Aplicar parches",
    description: "Aplicar cambios parciales a archivos existentes",
  },
  grep: {
    label: "Buscar en el código",
    description: "Buscar texto o patrones en los archivos del proyecto",
  },
  list_files: {
    label: "Listar archivos",
    description: "Ver el listado de archivos y directorios del proyecto",
  },
  code_search: {
    label: "Búsqueda en código fuente",
    description: "Buscar símbolos, funciones o clases en el código",
  },
};
...
export function AgentToolsSettings() {
  ...
  const handleConsentChange = (toolName: string, consent: AgentToolConsent) => {
    ...
    <div className="space-y-1">
      {allTools.map((tool: AgentTool) => (
        <ToolConsentRow
          ...
          onConsentChange={(consent) => handleConsentChange(tool.name, consent)}
        />
      ))}
    </div>
  );
  ...
  return (
    <div className="flex justify-between gap-8 p-4 rounded-xl hover:bg-muted/50 transition-colors items-center">
      <div className="flex-1 min-w-0">
        <h3 className="typo-section-title text-foreground">{label}</h3>
        {description && (
