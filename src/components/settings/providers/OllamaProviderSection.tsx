...
export function OllamaProviderSection() {
  ...
  const checkStatus = useCallback(async () => {
    ...
    try {
      ...
      setStatus({
        online: false,
        modelCount: 0,
        url: urlInput || "http://localhost:11434",
      });
      setModels([]);
    }
    setChecking(false);
  ...
  const handleSaveUrl = async () => {
    await updateSettings(
      { ollamaBaseUrl: urlInput.trim() || undefined } as any,
      { showToast: true },
    );
        ...
        <div className="p-4 pt-0 space-y-4 border-t border-border bg-muted/10">
          ...
          <div className="flex items-center gap-2 pt-4">
            <span className="typo-caption text-muted-foreground">
              {status?.url || urlInput || "http://localhost:11434"}
            </span>
            <Button
              variant="ghost"
              size="sm"
          ...
          {models.length > 0 && (
            <div className="space-y-1.5">
              ...
              {models.map((m) => (
                <div
                  key={m.modelName}
                  className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-muted/20 border border-border/50"
                >
                  <div className="w-1.5 h-1.5 rounded-full bg-emerald-500/60 shrink-0" />
                  <span className="typo-select font-medium text-sm">
                    {m.displayName}
                  </span>
                  <span className="typo-caption text-muted-foreground/60 ml-auto font-mono text-xs">
                    {m.modelName}
                  </span>
                </div>
              ))}
