  ...
  function FilterChip<T extends string>({
    ...
    return (
      <div className="flex items-center gap-2">
        <span
          className="typo-menu-header opacity-60 shrink-0 uppercase tracking-wider"
    ...
    <>
      <Dialog open={open} onOpenChange={onOpenChange}>
        ...
        >
          ...
          <div className="px-6 py-3 border-t border-border/30 shrink-0 typo-caption opacity-60 text-center bg-muted/5">
            {filteredModels.length} de {allModels.length} modelos ·{" "}
            {enabledModels.length} habilitados
          </div>
        </DialogContent>
      </Dialog>
      ...
      {detailModel && (
        <ModelInfoDialog
          ...
          onSetAlias={(newAlias) =>
            setAlias({ modelId: detailModel.apiName, alias: newAlias })
          }
