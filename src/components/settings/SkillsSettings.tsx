                ...
                <div className="flex-1 flex items-center gap-3 min-w-0">
                  <Switch
                    ...
                    onCheckedChange={(checked) =>
                      onToggleEnabled(skill, scope, checked)
                    }
                  />
...
export function SkillsSettings() {
  ...
  const [projectsWithSkills, setProjectsWithSkills] = useState<
    { app: { id: number; name: string; path: string }; skills: SkillData[] }[]
  >([]);
      ...
      await Promise.all(
        mappedApps.map(async (app) => {
          try {
            ...
            const skillFiles = (appData.files || []).filter(
              (f) =>
                f.startsWith(".claude/skills/") &&
                (f.endsWith("/SKILL.md") || f.endsWith("/SKILL.disabled")),
            );
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (scope === "global") {
        ...
        if (
          existingSkill &&
          (existingSkill.name !== skillSlug || existingScope !== scope)
        ) {
          if (existingScope === "global") {
            await ipc.settings
              .deleteGlobalSkill({ filePath: existingSkill.name })
              .catch(() => {});
          } else {
            await ipc.app
              .deleteAppFile({
                appId: Number(existingScope),
                filePath: `.claude/skills/${existingSkill.name}`,
              })
              .catch(() => {});
          }
        }
