    ...
    <div className="pt-3">
      <div className="typo-micro uppercase tracking-wider flex items-center justify-between mb-3">
        ...
        <button
          ...
          className="hover:text-primary transition-colors hover:bg-primary/10 p-1 rounded"
        >
      ...
      <div className="flex flex-wrap gap-2">
        {tools.map((tool) => (
          <div
            key={tool.name}
            className="px-2.5 py-1 typo-mono-xs border border-border/80 bg-background/50 rounded-md hover:bg-background/80 transition-colors"
...
function McpServerDialog({ existingServer }: { existingServer?: McpServer }) {
  ...
  const [transport, setTransport] = useState<"stdio" | "http">(
    existingServer?.transport === "http" || existingServer?.transport === "sse"
      ? "http"
      : "stdio",
  );
  ...
  if (existingServer?.args) {
    try {
      initialArgs = (
        typeof existingServer.args === "string"
          ? JSON.parse(existingServer.args)
          : existingServer.args
      ).join("\n");
    } catch (e) {}
  }
  ...
  if (existingServer?.envJson) {
    try {
      const parsed =
        typeof existingServer.envJson === "string"
          ? JSON.parse(existingServer.envJson)
          : existingServer.envJson;
      initialEnv = Object.entries(parsed)
        .map(([k, v]) => `${k}=${v}`)
        .join("\n");
    } catch (e) {}
  }
  ...
  if (existingServer?.headersJson) {
    try {
      const parsed =
        typeof existingServer.headersJson === "string"
          ? JSON.parse(existingServer.headersJson)
          : existingServer.headersJson;
      initialHeaders = Object.entries(parsed)
        .map(([k, v]) => `${k}=${v}`)
        .join("\n");
    } catch (e) {}
  }
      ...
      <DialogContent className="sm:max-w-[650px]">
        ...
        <DialogFooter>
          ...
          <Button
            ...
            disabled={
              (!isContext7 &&
                (!name || (transport === "stdio" ? !command : !url))) ||
              isCreating ||
              isUpdating
            }
          >
