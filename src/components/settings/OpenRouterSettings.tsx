...
import { KeyRound, Trash2, Clipboard, AlertTriangle, ExternalLink } from "lucide-react";
...
export function OpenRouterSettings({ isHighlighted }: { isHighlighted?: boolean }) {
    const {
        settings,
        envVars,
        ...
        >
            <div className="flex items-center justify-between mb-8">
                ...
                {providerData?.websiteUrl && (
                    ...
                    >
                        <a href={providerData.websiteUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 font-bold uppercase tracking-widest text-[10px]">
                        ...
                        {isValidUserKey ? (
                            <Alert className="bg-primary/5 border-primary/20 rounded-2xl p-6">
                                ...
                                <div className="flex justify-between items-center w-full">
                                    <div className="ml-3">
                                        <AlertTitle className="text-base font-bold text-primary">Clave Activa</AlertTitle>
                                        <AlertDescription className="font-mono text-sm opacity-70 mt-1">
                                            {userApiKey.substring(0, 8)}...{userApiKey.substring(userApiKey.length - 4)}
                                        </AlertDescription>
                                    </div>
                                    <Button
                                        variant="ghost"
                            ...
                            <Alert className="bg-green-500/5 border-green-500/20 rounded-2xl p-6">
                                ...
                                <div className="ml-3">
                                    <AlertTitle className="text-base font-bold text-green-600">Usando Variable de Entorno</AlertTitle>
                                    <AlertDescription className="text-sm opacity-70 mt-1">
                                        Se ha detectado una clave en la variable <code>{envVarName}</code>.
                                    </AlertDescription>
                                </div>
                            </Alert>
                        ) : (
                            <Alert className="bg-amber-500/5 border-amber-500/20 rounded-2xl p-6">
                                ...
                                <div className="ml-3">
                                    <AlertTitle className="text-base font-bold text-amber-600">Configuración Pendiente</AlertTitle>
                                    <AlertDescription className="text-sm opacity-70 mt-1">
                                        Necesitas una clave API de OpenRouter para usar las funciones de IA.
                                    </AlertDescription>
                                </div>
                            </Alert>
                        )}
                            ...
                            <div className="relative flex-1">
                                <Input
                                    ...
                                    className={cn(
                                        ...
                                        saveError && "border-destructive focus-visible:ring-destructive/20"
                                    )}
                                />
                            </div>
                            <Tooltip>
