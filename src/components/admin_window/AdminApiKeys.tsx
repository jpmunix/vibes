    ...
    try {
      ...
      const results = await Promise.all(
        usersResult.users.map(async (user) => {
          try {
            const result = await ipc.admin.getUserPreferences({
              userId: user.id,
            });
            return {
              userId: user.id,
...
function ApiKeyRow({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await ipc.admin.setUserPreference({
        userId,
        key: keyEntry.prefKey,
        value: wrappedValue,
      });
