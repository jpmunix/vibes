...
function PayloadBlock({
  ...
  const labelColor =
    variant === "error" ? "text-rose-500" : "text-muted-foreground";
  const textColor =
    variant === "error"
      ...
      <div className="bg-card rounded-2xl shadow-sm p-8 border border-border">
        <div className="mb-8">
          ...
          <p className="typo-caption mt-1">
            Memorias, métricas y debug logs del pipeline de todos los usuarios
          </p>
        </div>
        <div className="space-y-4">
          {data.length === 0 ? (
            <p className="typo-caption text-muted-foreground">
              Sin datos de conocimiento registrados.
            </p>
          ) : (
            data.map((user) => {
              ...
              return (
                <div key={user.userId}>
                  ...
                  <div
                    ...
                    onClick={() => {
                      setExpandedUserId(isUserOpen ? null : user.userId);
                  ...
                  >
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="typo-caption mt-0.5">
                        ...
                        {user.apps.length !== 1 ? "s" : ""} · {totalMems}{" "}
                        memorias
                      </p>
                    </div>
                    <ChevronRight
                        ...
                        return (
                          <div key={app.appId}>
                            ...
                            <div
                              ...
                              onClick={() =>
                                setExpandedAppId(isAppOpen ? null : app.appId)
                              }
                            >
                              <div className="flex-1 min-w-0">
                                ...
                                <p className="typo-caption mt-0.5">
                                  {app.enabled} activas
                                  {app.disabled > 0
                                    ? ` · ${app.disabled} desactivadas`
                                    : ""}{" "}
                                  · {app.autoCount} automáticas ·{" "}
                                  {app.manualCount} manuales
                                </p>
                              </div>
                              <ChevronRight
                                className={cn(
...
function AppAnalyzer({ userId, appId }: { userId: string; appId: number }) {
  ...
  useEffect(() => {
    setIsLoading(true);
    ipc.admin
      .getAdminAnalyzerData({ userId, appId })
      .then((r) => {
        setStats(r.stats);
        setRecent(r.recent);
        setPipelineLogs(r.pipelineLogs);
      })
      .catch(() => {})
  ...
  useEffect(() => {
    ...
    ipc.admin
      .getAdminDebugLogs({ userId, appId, limit: 500 })
      .then(setDebugLogs)
      .catch(() => {})
  ...
  const skippedCount = useMemo(
    () =>
      stats
        .filter((s) => s.action.startsWith("skipped_"))
  ...
  const evaluatedPct =
    totalInteractions > 0
      ? Math.round((evaluatedCount / totalInteractions) * 100)
      : 0;
  const savedPct =
    evaluatedCount > 0
      ? Math.round((synthesizedCount / evaluatedCount) * 100)
      : 0;
  const recentDiscards = useMemo(
    () =>
      recent
        .filter(
          (r) =>
            r.action.startsWith("skipped_") || r.action === "discarded_quality",
        )
        .slice(0, 5),
    [recent],
  );
  const recentOps = useMemo(
    () =>
      recent
        .filter((r) =>
          ["synthesized", "overwritten", "merged"].includes(r.action),
        )
        .slice(0, 5),
    [recent],
  );
  ...
  if (!hasAnyData) {
    return (
      <div className="p-4 pl-8">
        <p className="typo-caption text-muted-foreground">
          Sin datos de análisis para esta app.
        </p>
      </div>
    );
  }
    ...
    <div className="pl-4 mt-2 space-y-4 mb-2">
        ...
        totalInteractions === 0 ? (
          ...
          <>
            ...
            <div className="p-4 rounded-xl border border-border space-y-3">
              <h4 className="typo-label text-muted-foreground">
              ...
              />
              <FunnelBar
                label="Evaluadas (pasaron guardián)"
                value={`${evaluatedCount} (${evaluatedPct}%)`}
                pct={evaluatedPct}
                color="bg-blue-500"
              />
              <FunnelBar
                label="Guardadas"
                value={`${synthesizedCount} (${savedPct}%)`}
                pct={
                  totalInteractions > 0
                    ? Math.round((synthesizedCount / totalInteractions) * 100)
                    : 0
                }
            ...
            <div className="p-4 rounded-xl border border-border space-y-2">
              ...
              {stats.map((s) => {
                const m = ACTION_LABELS[s.action] || {
                  label: s.action,
                  color: "bg-gray-400",
                };
                return (
                  ...
                  >
                    <div
                      className={cn("w-2 h-2 rounded-full shrink-0", m.color)}
                    />
            ...
            {recentDiscards.length > 0 && (
              <div className="p-4 rounded-xl border border-border space-y-2">
                ...
                {recentDiscards.map((r, i) => {
                  const m = ACTION_LABELS[r.action] || {
                    label: r.action,
                    color: "bg-gray-400",
                  };
                  return (
                    <div
                      key={i}
                      className="flex items-start gap-2 typo-caption"
                    >
                      <div
                        className={cn(
            ...
            {recentOps.length > 0 && (
              <div className="p-4 rounded-xl border border-border space-y-2">
                ...
                {recentOps.map((r, i) => {
                  ...
                  let keys: string[] = [];
                  try {
                    keys = r.extractedKeys ? JSON.parse(r.extractedKeys) : [];
                  } catch {
                    /* */
                  }
                  return (
          ...
          <div className="space-y-2">
            {pipelineLogs.map((log) => (
              <PipelineLogRow
                ...
                fullPayload={fullPayloadLogId === log.id}
                onTogglePayload={() =>
                  setFullPayloadLogId(
                    fullPayloadLogId === log.id ? null : log.id,
                  )
                }
              />
            ))}
      ...
      debugLoading ? (
        ...
        <p className="typo-caption p-4">
          Sin debug logs para esta app. Los debug logs solo se generan en
...
function PipelineLogRow({
  ...
  try {
    if (log.parsedResult) {
      const p = JSON.parse(log.parsedResult);
      meta = p?.meta || (p?.rejectReason ? p : null);
    }
  } catch {
    /* */
  }
  ...
  if (log.stage === "guardian" && meta?.rejectReason) {
    ...
    chip = (
      <span className="typo-micro px-1.5 py-0.5 rounded-md bg-muted/50 text-muted-foreground shrink-0">
        {meta.existingMemoriesCount} exist →{" "}
        {meta.operationsGenerated ?? log.resultCount} ops
      </span>
    );
  ...
  return (
    <div className="rounded-xl border border-border overflow-hidden">
      ...
      >
        ...
        <span
          className={cn(
            "typo-caption font-medium shrink-0 w-20",
            stageMeta.color,
          )}
        >
          {stageMeta.label}
        ...
        <div className="border-t border-border p-3 space-y-3 bg-muted/20">
          ...
          {log.parsedResult && (
            <PayloadBlock
              ...
              content={(() => {
                try {
                  return JSON.stringify(JSON.parse(log.parsedResult), null, 2);
                } catch {
                  return log.parsedResult;
                }
              })()}
              expanded={fullPayload}
            />
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <p className="typo-caption font-medium truncate">
                {log.appName || log.filename}
              </p>
              <p className="typo-micro text-muted-foreground">
                {log.filename} · {(log.contentMd?.length ?? 0).toLocaleString()}{" "}
                chars
              </p>
            </div>
            <span className="typo-micro text-muted-foreground shrink-0">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-border">
              <div>
                <h3 className="typo-body font-semibold">
                  {viewingLog.appName || viewingLog.filename}
                </h3>
                <p className="typo-micro text-muted-foreground">
                  {viewingLog.filename} · {formatTime(viewingLog.createdAt)} ·{" "}
                  {(viewingLog.contentMd?.length ?? 0).toLocaleString()} chars
                </p>
              </div>
              <div className="flex items-center gap-2">
                <Button
                  ...
                  onClick={() =>
                    downloadMarkdown(viewingLog.filename, viewingLog.contentMd)
                  }
                >
                  <Download className="size-3.5" />
                  Descargar
                ...
                <Button
                  ...
                  size="sm"
                  className="gap-1.5 cursor-pointer"
                  onClick={() => handleShare(viewingLog)}
                >
                  <Share2 className="size-3.5" />
                  Compartir
                </Button>
