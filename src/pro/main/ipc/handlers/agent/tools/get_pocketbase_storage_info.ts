  ...
  execute: async (args, ctx: AgentContext) => {
    ...
    try {
      ...
      if (!args.collectionName) {
        ...
        const fileFields = (col.fields || col.schema || [])
          .filter((f: any) => f.type === "file")
          .map((f: any) => f.name);
