...
const askUserSchema = z.object({
  ...
  options: z
    ...
    .optional()
    .describe(
      "Optional list of predefined options the user can choose from. If not provided, the user can type a free-form response.",
    ),
  context: z
    .string()
    .optional()
    .describe(
      "Optional additional context explaining why you need this information.",
    ),
});
...
- Prefer using 'options' when there are a finite set of valid choices.`,
  ...
  execute: async (args, ctx: AgentContext): Promise<ToolResult> => {
    try {
      ...
      ctx.onXmlComplete(
        `<vibes-ask-user ${closeAttrs.join(" ")}>${escapeXmlAttr(response)}</vibes-ask-user>`,
      );

      if (!response) {
        throw new ToolError(
          "The user did not provide a response (stream was cancelled).",
          {
            retryable: false,
          },
        );
