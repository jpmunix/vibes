...
function validateExecutable(cmd: string): void {
  ...
  if (!ALLOWED_EXECUTABLES.has(basename)) {
    throw new ToolError(
      ...
      {
        retryable: false,
        hint: `Allowed: ${[...ALLOWED_EXECUTABLES].join(", ")}`,
      },
    );
  }
}
...
function validateCwd(cwd: string, projectRoot: string): string {
  ...
  if (!resolved.startsWith(projectRoot)) {
    throw new ToolError(`El directorio '${cwd}' está fuera del proyecto.`, {
      retryable: false,
    });
  }
  return resolved;
...
The process runs in background until stopped with stop_process. Use list_processes to see running processes.`,
  ...
  execute: async (args, ctx: AgentContext) => {
    ...
    const cwd = args.cwd ? validateCwd(args.cwd, ctx.appPath) : ctx.appPath;
    ...
    if (args.ready.type !== "none") {
      ...
      if (!isReady) {
        logger.warn(
          `Process ${proc.id} did not become ready within ${args.ready.timeout_ms}ms`,
        );
      }
    }
...
const stopProcessSchema = z.object({
  process_id: z.string().describe("The process ID returned by start_process."),
  signal: z
...
> = {
  ...
  getConsentPreview: (args) => `Detener proceso: ${args.process_id}`,
...
const listProcessesSchema = z.object({
  _placeholder: z
    .string()
    .optional()
    .describe("Not used. This tool takes no arguments."),
});
