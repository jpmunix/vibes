...
- "patch": Line-number-based edits. Requires reading the file first with read_file to know line numbers. Best for precise multi-region edits.`,
  ...
  search_replace: searchReplaceOperationSchema
    .optional()
    .describe(
...
function quickSyntaxCheck(content: string, filePath: string): string | null {
  ...
  const importRegex =
    /^\s*(?:}\s*from\s+|import\s+.+?from\s+)['"]([^'"]+)['"];?\s*$/gm;
  ...
  if (Math.abs(braceDepth) >= 3) {
    ...
    logger.warn(
      `[quickSyntaxCheck] Severe brace imbalance (${braceDepth}) in ${filePath} — likely a real issue but not blocking the edit`,
    );
  }
  if (Math.abs(parenDepth) >= 3) {
    logger.warn(
      `[quickSyntaxCheck] Severe paren imbalance (${parenDepth}) in ${filePath}`,
    );
  }
...
async function autoTypeCheck(
  ...
  try {
    ...
    const fileProblems = report.problems.filter((p) => {
      ...
      return (
        normalizedProblem === normalizedFilePath ||
...
export const fileEditorTool: ToolDefinition<FileEditorArgs> = {
  ...
  buildXml: (args, isComplete, ctx) => {
    ...
    const desc = escapeXmlAttr(args.description ?? args.instructions ?? "");
  ...
  execute: async (args, ctx: AgentContext) => {
    ...
    switch (args.action) {
      ...
      case "overwrite": {
        if (!args.content) {
          throw new ToolError("'content' is required for action 'overwrite'.", {
            retryable: true,
          });
        }
        ...
        ctx.typecheckResults.push({
          file: args.file_path,
          status: typeErrors ? "error" : "ok",
          errors: typeErrors ? [typeErrors] : [],
        });
        return (
          (deployMsg ?? `Successfully overwrote ${args.file_path}`) + typeErrors
        );
      }
      ...
      case "edit": {
        if (!args.edit_content) {
          throw new ToolError("'edit_content' is required for action 'edit'.", {
            retryable: true,
          });
        }
        ...
        if (editSyntaxError) {
          ...
          logger.warn(
            `Syntax warning after edit on ${args.file_path}: ${editSyntaxError}`,
          );
        }
        ...
        return (
          (deployMsg ?? `Successfully edited ${args.file_path}`) +
          editWarning +
          typeErrors
        );
      }
      ...
      case "search_replace": {
        ...
        return (
          (deployMsg ?? `Successfully applied edits to ${args.file_path}`) +
          typeErrors
        );
      }
      ...
      // ────────────────────────────────────────────────
      case "patch": {
        if (!args.patch_operations || args.patch_operations.length === 0) {
          throw new ToolError(
        ...
        if (ctx?.fileEditTracker?.[args.file_path]) {
          const patchCount =
            ctx.fileEditTracker[args.file_path].patch_file ?? 0;
          if (patchCount >= 2) {
            throw new ToolError(
