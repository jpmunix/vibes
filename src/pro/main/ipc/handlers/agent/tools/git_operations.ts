...
- "revert_file": Discard changes in a specific file`),
  ...
  cached: z
    ...
    .optional()
    .describe(
      "For 'diff': if true, show staged diff instead of working directory diff.",
    ),
  ...
  limit: z
    ...
    .optional()
    .describe(
      "For 'log': maximum number of commits to return (default: 10, max: 50).",
    ),
  offset: z
    .number()
    .int()
    ...
    .optional()
    .describe(
      "For 'log': number of commits to skip for pagination (default: 0).",
    ),
  ...
  from: z
    ...
    .optional()
    .describe(
      "For 'create_branch': starting point for the branch (default: HEAD).",
    ),
...
async function executeStatus(ctx: AgentContext): Promise<ToolResult> {
  ...
  const lines = [
    `On branch: ${branch ?? "(detached HEAD)"}`,
...
async function executeDiffFile(
  ctx: AgentContext,
  filePath: string,
): Promise<ToolResult> {
...
async function executeLog(
  ...
  const lines = [
    `Showing ${result.commits.length} of ${result.total} commits:`,
  ];
  for (const c of result.commits) {
    const filesStr =
      c.filesChanged > 0
        ...
        : "";
    lines.push(
      `  ${c.shortHash} ${c.message} (${c.author}, ${c.date})${filesStr}`,
    );
  }
  if (result.hasMore) {
...
async function executeShowCommit(
  ctx: AgentContext,
  commitHash: string,
): Promise<ToolResult> {
...
async function executeStashSave(
  ctx: AgentContext,
  message?: string,
): Promise<ToolResult> {
...
async function executeRevertFile(
  ctx: AgentContext,
  filePath: string,
): Promise<ToolResult> {
...
Use descriptive commit messages following conventional commit format (e.g. "feat: add login page").`,
  ...
  buildXml: (args, isComplete) => {
    ...
    switch (args.operation) {
      ...
      case "show_commit":
        if (args.commit_hash)
          attrs.push(`commit="${escapeXmlAttr(args.commit_hash)}"`);
        break;
      case "commit":
        if (args.message)
          attrs.push(`message="${escapeXmlAttr(args.message)}"`);
        break;
      case "create_branch":
  ...
  execute: async (args, ctx: AgentContext): Promise<ToolResult> => {
    ...
    if (!READ_OPERATIONS.has(args.operation)) {
      const preview =
        gitOperationsTool.getConsentPreview?.(args) ?? args.operation;
      ...
      if (!allowed) {
        throw new ToolError(
          `User denied permission for git ${args.operation}`,
          {
            retryable: false,
          },
        );
    ...
    try {
      switch (args.operation) {
        ...
        default:
          throw new ToolError(
            `Unknown git operation: ${(args as any).operation}`,
            {
              retryable: false,
            },
          );
