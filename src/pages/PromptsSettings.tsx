...
import { ArrowLeft, RotateCcw, Save, Search, Sparkles, MessageSquare, Terminal, Lightbulb, ShieldCheck, Zap, AlertTriangle, FileSearch, Wand2 } from "lucide-react";
...
import { DEFAULT_PROMPTS, PROMPT_LABELS, PROMPT_DESCRIPTIONS, PromptId } from "@/prompts";
        ...
        <div className="flex flex-col w-full min-h-full bg-background text-foreground">
                ...
                <div className="flex items-center gap-4 flex-1">
                    ...
                    <div className="flex items-center flex-1">
                        <div className="shrink-0">
                            <h1 className="text-xl font-semibold tracking-tight">Configuración de Prompts</h1>
                            <p className="text-sm text-muted-foreground">Personaliza el comportamiento del asistente AI</p>
                        </div>

                        <Alert variant="destructive" className="bg-destructive/5 border-destructive/20 py-2 mx-12 flex-1">
                            <AlertTriangle className="w-4 h-4 text-destructive" />
                            <AlertDescription className="text-[11px] leading-tight text-destructive/90">
                                <strong>Opción avanzada:</strong> No elimines tags vitales como [[AI_RULES]] o [[LANGUAGE_INSTRUCTION]] o tag tipo html. Su permanencia es necesaria para el correcto funcionamiento del sistema.
                            </AlertDescription>
                        </Alert>
                    </div>
                </div>
            ...
            <div className="flex flex-1">
                ...
                <div className={cn(
                    ...
                    sidebarCollapsed ? "w-0 opacity-0 pointer-events-none border-0" : "w-110"
                )}>
                        ...
                        {filteredPrompts.map((id) => (
                            ...
                            >
                                <div className={cn(
                                    ...
                                    activeTab === id ? "bg-primary text-primary-foreground" : "bg-muted group-hover:bg-muted-foreground/10"
                                )}>
                                    {PROMPT_ICONS[id]}
                                </div>
                                <div className="flex-1 overflow-hidden">
                                    <div className="text-sm font-medium truncate">{PROMPT_LABELS[id]}</div>
                ...
                <div className="flex-1 flex flex-col bg-background relative">
                    ...
                    {activeTab ? (
                        <div className="flex flex-col bg-gradient-to-b from-transparent to-muted/20">
                            ...
                            <div className="px-6 pt-6 pb-4 flex items-start justify-between gap-4">
                                ...
                                <div>
                                    ...
                                    <h2 className="text-2xl font-bold">{PROMPT_LABELS[activeTab]}</h2>
                                    <p className="text-muted-foreground mt-1">{PROMPT_DESCRIPTIONS[activeTab]}</p>
                                ...
                                <div className="flex gap-2 shrink-0">
                                    ...
                                    <Button
                                        ...
                                        disabled={loading || localPrompts[activeTab] === DEFAULT_PROMPTS[activeTab]}
                                    >
                                    ...
                                    <Button
                                        ...
                                        disabled={loading || localPrompts[activeTab] === settings?.customPrompts?.[activeTab]}
                                    >
                            ...
                            <div className="px-4 pb-4 flex flex-col">
                                <div className="rounded-xl border bg-card shadow-sm flex flex-col overflow-hidden">
                                    ...
                                    <div className="flex items-center justify-between px-4 py-2 border-b bg-muted/30">
                                        <span className="text-xs font-mono text-muted-foreground">Original length: {DEFAULT_PROMPTS[activeTab].length} chars</span>
                                        {localPrompts[activeTab] !== DEFAULT_PROMPTS[activeTab] && (
                                            <span className="text-[10px] px-1.5 py-0.5 rounded bg-amber-100 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400 font-medium">MODIFICADO</span>
                                        )}
                                    </div>
                                    <Textarea
                                        ref={textareaRef}
                        ...
                        <div className="flex-1 flex flex-col items-center justify-center text-center p-12 opacity-50">
                            ...
                            <h3 className="text-xl font-medium">Selecciona un prompt para editar</h3>
                            <p className="max-w-xs mt-2 text-sm">Escoge una de las etiquetas en la izquierda para personalizar las instrucciones que recibe la IA.</p>
                        </div>
                    )}
                </div>
            </div>
