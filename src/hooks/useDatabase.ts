...
export function useDatabase() {
  ...
  const tableDataQuery = useQuery({
    queryKey:
      dbType === "bunny"
        ...
        : dbType === "pocketbase"
          ? queryKeys.pocketbase.dbTableData(
              appId,
              selectedTable ?? "",
              page,
              pageSize,
          ...
          : queryKeys.supabase.dbTableData(
              appId,
              selectedTable ?? "",
              page,
              pageSize,
  ...
  const selectedTableInfo = useMemo(() => {
    ...
    return (
      (tablesQuery.data.tables as any[]).find(
  ...
  const executeQueryMutation = useMutation({
    mutationFn: (query: string) => {
      ...
      if (dbType === "pocketbase")
        return ipc.pocketbase.executeQuery({ appId, query });
