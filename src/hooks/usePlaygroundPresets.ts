...
export function usePlaygroundPresets() {
    ...
    (async () => {
      try {
        ...
        if (raw[MODEL_PRESETS_KEY]) {
          try {
            setModelPresetsState(JSON.parse(raw[MODEL_PRESETS_KEY]));
          } catch {
            /* corrupt */
          }
        }
        if (raw[PROMPT_PRESETS_KEY]) {
          try {
            setPromptPresetsState(JSON.parse(raw[PROMPT_PRESETS_KEY]));
          } catch {
            /* corrupt */
          }
        }
  ...
  const saveModelPreset = useCallback(
    async (name: string, models: string[]) => {
      ...
      const next = [
        ...modelPresets.filter((p) => p.name !== trimmed),
        { name: trimmed, models },
      ];
  ...
  const renameModelPreset = useCallback(
    async (oldName: string, newName: string) => {
      const trimmed = newName.trim();
      if (!trimmed || trimmed === oldName) return;
      const next = modelPresets.map((p) =>
  ...
  const savePromptPreset = useCallback(
    async (name: string, prompt: string) => {
      ...
      const next = [
        ...promptPresets.filter((p) => p.name !== trimmed),
        { name: trimmed, prompt },
      ];
  ...
  const renamePromptPreset = useCallback(
    async (oldName: string, newName: string) => {
      const trimmed = newName.trim();
      if (!trimmed || trimmed === oldName) return;
      const next = promptPresets.map((p) =>
        p.name === oldName ? { ...p, name: trimmed } : p,
      );
      await persistPromptPresets(next);
    },
  ...
  const migrateFromSettings = useCallback(
    async (settingsModelSets: ModelPreset[] | undefined) => {
