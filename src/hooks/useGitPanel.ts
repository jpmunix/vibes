...
export function useGitPanel(appId: number | null) {
    ...
    (selectedFiles?: GitPanelFile[]) => {
      ...
      renderInterval = setInterval(() => {
        if (renderQueue.length > 0) {
          ...
          const chars = renderQueue.splice(
            0,
            Math.max(1, Math.ceil(renderQueue.length / 15)),
          );
      ...
      const filesToPass = (selectedFiles || uncommittedFiles).map((f) => ({
        path: f.path,
        status: f.status,
      }));
      window.electron.ipcRenderer
        .invoke("github:generate-commit-message-stream", {
          appId,
          files: filesToPass,
        })
        .catch((err: any) => {
  ...
  const abortMergeMutation = useMutation({
    ...
    onError: (err: Error) =>
      toast.error(`Error al abortar merge: ${err.message}`),
  });
  ...
  const resolveFileOursMutation = useMutation({
    ...
    onError: (err: Error) =>
      toast.error(`Error al resolver archivo: ${err.message}`),
  });
  ...
  const switchBranchMutation = useMutation({
    ...
    onError: (err: Error) =>
      toast.error(`Error al cambiar de rama: ${err.message}`),
  });
  ...
  return {
    ...
    isGeneratingMessage,
    isResolvingMerge:
      resolveMergeOursMutation.isPending ||
      resolveMergeTheirsMutation.isPending,
    isAbortingMerge: abortMergeMutation.isPending,
    isResolvingFile:
      resolveFileOursMutation.isPending || resolveFileTheirsMutation.isPending,
    isSwitchingBranch: switchBranchMutation.isPending,
    isDiscarding:
      discardFileChangesMutation.isPending ||
      discardAllChangesMutation.isPending,
  };
}
