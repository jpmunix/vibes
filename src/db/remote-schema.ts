...
export const labels = sqliteTable(
  ...
  (table) => ({
    userLabelUniqueIdx: uniqueIndex("idx_labels_user_name").on(
      table.userId,
      table.name,
    ),
  }),
...
export const chatArtifactsRelations = relations(
  chatArtifacts,
  ({ one, many }) => ({
    user: one(users, {
      fields: [chatArtifacts.userId],
      references: [users.id],
    }),
    app: one(apps, { fields: [chatArtifacts.appId], references: [apps.id] }),
    chat: one(chats, {
      fields: [chatArtifacts.chatId],
      references: [chats.id],
    }),
...
export const artifactCommentsRelations = relations(
  artifactComments,
  ({ one }) => ({
    artifact: one(chatArtifacts, {
      fields: [artifactComments.artifactId],
      references: [chatArtifacts.id],
    }),
    user: one(users, {
      fields: [artifactComments.userId],
      references: [users.id],
    }),
  }),
);
...
export const streamTasksRelations = relations(streamTasks, ({ one }) => ({
  ...
  message: one(messages, {
    fields: [streamTasks.messageId],
    references: [messages.id],
  }),
}));

export const promptsCategoriesRelations = relations(
  promptsCategories,
  ({ one, many }) => ({
    user: one(users, {
      fields: [promptsCategories.userId],
      references: [users.id],
    }),
...
export const promptsRelations = relations(prompts, ({ one }) => ({
  ...
  category: one(promptsCategories, {
