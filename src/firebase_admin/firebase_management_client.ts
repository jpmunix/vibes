...
export async function refreshFirebaseToken(): Promise<void> {
  ...
  if (!refreshToken) {
    throw new Error(
      "Firebase refresh token not found. Please authenticate first.",
    );
  }
...
export async function getFirebaseAccessToken(): Promise<string> {
  ...
  if (!firebaseAccessToken) {
    throw new Error(
      "Firebase access token not found. Please authenticate first.",
    );
  }
  ...
  for (let i = 0; i < maxRetries; i++) {
    try {
      ...
      if (response.status === 404 || response.status === 403) {
        ...
        logger.info(
          `Source not ready (Status ${response.status}), retrying in ${delay}ms... (Attempt ${i + 1}/${maxRetries})`,
        );
...
export async function createFirebaseWebApp(
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Failed to create Firebase web app: ${errBody.error?.message || response.statusText}`,
    );
  }
...
export async function getFirebaseProjectWebConfig(
  projectId: string,
  appId?: string,
  displayName?: string,
) {
  ...
  if (!targetAppId) {
    ...
    const appsResponse = await fetchWithRetry(
      `https://firebase.googleapis.com/v1beta1/projects/${projectId}/webApps`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
    ...
    if (!appsResponse.ok) {
      throw new Error(
        `Failed to list Firebase web apps: ${appsResponse.statusText} (${appsResponse.status})`,
      );
    }
    ...
    if (!webApp) {
      ...
      if (!createResponse.ok) {
        ...
        throw new Error(
          `Failed to initiate Firebase web app creation: ${errBody.error?.message || createResponse.statusText}`,
        );
      }

      // Poll for the app to be available (it can take up to 20-30 seconds in fresh projects)
      logger.info(
        `Web app creation initiated for ${projectId}. Polling for availability...`,
      );
      let attempts = 0;
      ...
      while (attempts < maxAttempts) {
        attempts++;
        logger.info(
          `Polling for web app in ${projectId} (Attempt ${attempts}/${maxAttempts})...`,
        );

        await new Promise((resolve) => setTimeout(resolve, delayMs));

        try {
          const pollFetch = await fetch(
            `https://firebase.googleapis.com/v1beta1/projects/${projectId}/webApps`,
            {
          ...
          if (pollFetch.ok) {
            ...
            if (webApp) {
              logger.info(
                `Web app found for ${projectId} after ${attempts} attempts!`,
              );
              break;
            }
          }
      ...
      if (!webApp) {
        throw new Error(
          "El proyecto de Firebase se ha creado, pero Google todavía no ha terminado de configurar la App web. Por favor, espera un minuto y vuelve a intentarlo seleccionando el proyecto de la lista.",
        );
      }
    }
  ...
  if (!configResponse.ok) {
    ...
    logger.error(
      `Failed to get Firebase web app config. Status: ${configResponse.status} ${configResponse.statusText}. Body: ${errorText}`,
    );
    throw new Error(
      `Failed to get Firebase web app config: ${configResponse.statusText} (${configResponse.status})`,
    );
  }
...
export async function createFirebaseProject(
  ...
  if (!gcpResponse.ok) {
    ...
    throw new Error(
      `Failed to create GCP project: ${error.error?.message || gcpResponse.statusText}`,
    );
  }
  ...
  if (!firebaseResponse.ok) {
    ...
    throw new Error(
      `Failed to add Firebase to project: ${error.error?.message || firebaseResponse.statusText}`,
    );
  }
...
export async function enableFirebaseHosting(projectId: string) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      logger.warn(
        `Could not automatically enable ${serviceName}: ${error.error?.message || response.statusText}`,
      );
    } else {
