...
export class PageObject {
  ...
  async clickFixErrorWithAI() {
    await this.page
      .getByRole("button", { name: "Arreglar el problema con la IA" })
      .click();
  }
  ...
  async clickFixAllErrors() {
    await this.page
      .getByRole("button", { name: /Arreglar todos los errores/ })
      .click();
  }
  ...
  getChatInput() {
    return this.page.locator(
      '[data-lexical-editor="true"][aria-placeholder^="Pídele a vibes"]',
    );
  }
  ...
  async goToAppsTab() {
    ...
    await expect(this.page.getByText("vibes.start()")).toBeVisible({
      timeout: 5000,
    });
  }
