...
<vibes-write path="new/image/file.png" description="Uploaded image to codebase">
VIBES_ATTACHMENT_0
</vibes-write>
`;
      messageContent += "\n\n" + generateDump(req);
    ...
    if (
      ...
      messageContent = `
<vibes-write path="src/bad-file.ts" description="Fix 2 errors and introduce a new error.">
...
x.nonExistentMethod2();
    ...
    if (
      ...
      // Fix errors in create-ts-errors.md and introduce a new error
      messageContent = `
<vibes-write path="src/bad-file.ts" description="Fix remaining error.">
// Import doesn't exist
// import NonExistentClass from 'non-existent-class';
...
x.toString(); // replaced with existing method
    ...
    if (
      ...
      messageContent = `
      Fixing the error...
      <vibes-write path="src/pages/Index.tsx">
      

import { MadeWithVibes } from "@/components/made-with-dyad";
...
export default Index;
    ...
    if (
      ...
      typeof lastMessage.content === "string" &&
      lastMessage.content.startsWith(
        "There was an issue with the following `vibes-search-replace` tags.",
      )
    ) {
      if (lastMessage.content.includes("Make sure you use `vibes-read`")) {
        // Fix errors in create-ts-errors.md and introduce a new error
        messageContent =
          `
<vibes-read path="src/pages/Index.tsx"></vibes-read>

<vibes-search-replace path="src/pages/Index.tsx">
<<<<<<< SEARCH
...
` +
        ...
        messageContent =
          `
<vibes-write path="src/pages/Index.tsx" description="Rewrite file.">
// FILE IS REPLACED WITH FALLBACK WRITE.
    ...
    if (
      ...
      messageContent = `[[STRING_IS_FINISHED]]";</vibes-write>\nFinished writing file.`;
      messageContent += "\n\n" + generateDump(req);
    }
    const isToolCall = !!(
      lastMessage &&
...
function generateDump(req: Request) {
  ...
  try {
    ...
    return `[[vibes-dump-path=${dumpFilePath}]]`;
  } catch (error) {
