...
/**
 * A webpack loader that adds `data-vibes-*` attributes to JSX elements.
 */
      ...
      walk(ast as any, {
        enter: (node: any) => {
          try {
            ...
            if (!loc) return;
            const vibesId = `${fileRelative}:${loc.line}:${loc.column}`;

            // Inject the attributes
            if (node.name.end != null) {
              ms.appendLeft(
                node.name.end,
                ` data-vibes-id="${vibesId}" data-vibes-name="${tagName}"`,
              );
              transformCount++;
            }
          } catch (error) {
